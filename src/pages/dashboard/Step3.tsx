...
const Step3 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  const [rows, setRows] = useState<AccessRow[]>(
    formData.accessList?.length
      ? formData.accessList
  ...
  const addRow = () => {
    const updated = [...rows, { fullName: "", email: "", accessLevel: "" }];
    setRows(updated);
    setFormData((prev) => ({ ...prev, accessList: updated }));
  };
      ...
      >
        ...
        <div className="flex justify-between mt-12">
          ...
          <button
            onClick={() => {
              if (isFormComplete) {
                setFormData((prev) => ({ ...prev, accessList: rows }));
                jumpToStep?.(4);
              }
            }}
