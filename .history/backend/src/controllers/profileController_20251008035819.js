...
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (onboard) {
      ...
      // Create new onboarding record with all required fields
      onboard = await Onboarding.create({
        companyInfo: {
          primaryName: name || "Unnamed User",
          primaryEmail: email,
          primaryPhone: phone || "",
          companyName: "Default Company Name", // required
        },
        avatar: avatarPath || "",
        agreements: [],    // supply required array
        awsSetup: false,   // supply required field
      });
    }
