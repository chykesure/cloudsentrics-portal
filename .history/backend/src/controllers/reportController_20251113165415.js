...
exports.createReport = async (req, res) => {
  try {
    ...
    const companyEmail =
      onboardingRecord?.companyInfo?.companyEmail?.trim() || email;
...
`;
    ...
    // Create Jira issue — ✅ summary now uses companyEmail
    const jiraResp = await createIssue({
      summary: `Issue Reported by (${companyEmail})`,
    ...
    // ------------------------------
    // ✅ Send acknowledgment email to company email
    // ------------------------------
    let recipientEmail = companyEmail; // use company email for acknowledgment
    ...
    if (recipientEmail) {
      const ackMailOptions = {
        ...
        to: recipientEmail,
        subject: `Your Issue Has Been Received – [Ticket #${ticketId}]`,
        html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; color: #333;">
          ...
          <div style="padding: 20px; line-height: 1.6;">
            <p>Dear ${fullName},</p>
            <p>Thank you for contacting <strong>Cloud Sentrics Limited</strong>.</p>
      ...
      try {
        ...
        console.log(`✅ Acknowledgment email sent to ${recipientEmail}:`, info.response);
      } catch (err) {
        console.error("❌ Error sending acknowledgment email:", err.message || err);
      }
    } else {
      console.warn("⚠️ No companyEmail or fallback email found — acknowledgment email skipped");
    }
