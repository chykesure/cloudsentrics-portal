...
exports.createReport = async (req, res) => {
  try {
    ...
    const cleanData = {
      ...
      bucketName: bucketName?.trim() || null,
      title: title?.trim(),
      description: description?.trim(),
      date,
      time,
      category,
      otherCategoryDesc: otherCategoryDesc?.trim(),
      steps: steps?.trim(),
      confirm,
    };
    ...
    // ✅ Save report to DB first
    const newReport = new Report({ ...cleanData, image: imageData });
    ...
    const descriptionADF = {
      ...
      version: 1,
      content: [
        { type: "paragraph", content: [{ type: "text", text: `Reporter: ${cleanData.fullName} (${cleanData.reporterEmail})` }] },
        { type: "paragraph", content: [{ type: "text", text: `Phone: ${cleanData.phone || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Company: ${cleanData.company || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Account ID: ${cleanData.accountId}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Bucket Name: ${cleanData.bucketName || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Title: ${cleanData.title || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Description: ${cleanData.description || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Date/Time: ${cleanData.date || "N/A"} / ${cleanData.time || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Category: ${cleanData.category || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Other Category: ${cleanData.otherCategoryDesc || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Steps Taken: ${cleanData.steps || "N/A"}` }] },
        { type: "paragraph", content: [{ type: "text", text: `Confirmed: ${cleanData.confirm ? "Yes" : "No"}` }] },
      ],
    };

    // ------------------------------
    // 6️⃣ Sync to Jira (inline)
    // ------------------------------
    ...
    // ------------------------------
    newReport.jiraIssueId = jiraResponse.data.id;
    newReport.jiraIssueKey = jiraResponse.data.key;
    newReport.jiraUrl = `${process.env.JIRA_BASE_URL}/browse/${jiraResponse.data.key}`;
    await newReport.save();
    ...
    res.status(201).json({
      ...
      report: newReport,
      jira: {
        key: newReport.jiraIssueKey,
        id: newReport.jiraIssueId,
        url: newReport.jiraUrl,
      },
    });
  } catch (err) {
