...
const getTierInfo = (latestRequest) => {
  ...
  // ✅ Fallback: derive from selectedTier
...
exports.getProfile = async (req, res) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ success: false, message: "Email not found in user record" });
    }
    ...
    // ✅ Always select the most recent valid tier request
    const latestRequest =
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (onboard) {
      ...
      onboard.companyInfo.companyName = companyName || onboard.companyInfo.companyName;
    ...
    // ✅ Always pick the most recent valid tier
    const latestRequest =
      (await Request.findOne({
        reporterEmail: email,
        selectedTier: { $exists: true, $ne: null },
