  ...
  try {
    ...
    const step4Content = step4Data
      ? [
          ...
          { type: "paragraph", content: [{ type: "text", text: `OTP Plan: ${
            Object.entries(step4Data.otpPlan || {}).map(([k,v]) => `${k}: ${v}`).join(", ") || "N/A"
          }` }] },
          { type: "paragraph", content: [{ type: "text", text: `Custom OTP: ${
            Object.entries(step4Data.customOtp || {}).map(([k,v]) => `${k}: ${v}`).join(", ") || "N/A"
          }` }] },
    ...
    // 3️⃣ Build Jira ADF with collapsible sections
    const jiraDescriptionADF = {
      ...
      content: [
        ...
        // ✅ Tier Section
        { type: "paragraph", content: [{ type: "text", text: "=== Tier Details ===", marks: [{ type: "strong" }] }] },
