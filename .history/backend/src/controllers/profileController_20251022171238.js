...
const getTierInfo = (latestRequest) => {
  if (!latestRequest) {
    return { title: "No tier selected", storage: "No Tier Selected" };
  }
  ...
  if (latestRequest.selectedTier) {
    ...
    else return { title: "No tier selected", storage: "No Tier Selected" };

    return tierMap[tierKey] || { title: "No tier selected", storage: "No Tier Selected" };
  }
...
exports.getProfile = async (req, res) => {
  try {
    ...
    const profile = {
      ...
      companyName: onboard?.companyInfo?.companyName || "No Company Name", // ✅ added fallback
