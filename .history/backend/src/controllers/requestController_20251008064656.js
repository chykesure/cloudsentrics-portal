    ...
    const jiraResponse = await axios.post(
      `${process.env.JIRA_BASE_URL}/rest/api/3/issue`,
      {
        fields: {
          ...
          summary: cleanData.title || `Request from ${cleanData.companyEmail}`,
          description: descriptionADF,
          issuetype: { name: "YOUR_EXACT_NAME_FROM_RESPONSE" },

          priority: { name: cleanData.priority || "Medium" },
          reporter: { emailAddress: cleanData.companyEmail }, // 👈 Key addition
        },
      },
      {
        auth: {
