...
const fetchAllJiraIssues = async () => {
  ...
  try {
    while (startAt < total) {
      // ✅ Using POST + /search/jql (new Jira API)
...
/**
 * GET /api/dashboard
 * Fetch Jira issues for the current user
 */
exports.getDashboard = async (req, res) => {
  try {
    // ✅ The frontend should send ?companyEmail=<email> in the query
    ...
    // ✅ Filter issues based on reporter email (matches companyEmail)
    ...
    console.log(
      `🎯 Issues found for ${userEmail}: ${filteredIssues.length}`
    );

    // Map filtered issues safely
    ...
    res
      .status(500)
      .json({
        message: "Failed to fetch Jira dashboard",
