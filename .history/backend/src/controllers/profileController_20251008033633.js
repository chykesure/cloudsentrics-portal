...
// ✅ GET profile by email (always returns a profile)
exports.getProfile = async (req, res) => {
  try {
    const email = req.params.email;

    // Fetch onboarding record (may not exist)
    ...
    const profile = {
      name: onboard?.companyInfo?.primaryName || email.split("@")[0], // fallback to email prefix
      companyEmail: onboard?.companyInfo?.primaryEmail || email,
...
// ✅ PUT update profile (updates or creates onboarding record)
exports.updateProfile = async (req, res) => {
  try {
    ...
    let onboard = await Onboarding.findOne({ "companyInfo.primaryEmail": email });

    if (onboard) {
      // Update existing onboarding
      onboard.companyInfo.primaryName = name;
      onboard.companyInfo.primaryPhone = phone;
