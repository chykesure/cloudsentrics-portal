...
const transporter = nodemailer.createTransport({
  ...
  tls: {
    rejectUnauthorized: false, // Helps prevent TLS rejection on Hostinger
  },
  logger: true,  // Logs detailed info for debugging
  debug: true,   // Enables SMTP-level debugging
});
...
`;
    ...
    // ------------------------------
    // ✅ Send acknowledgment email (to company email, not primary)
    // ------------------------------
    const companyEmail = report?.companyInfo?.companyEmail || null;
    const recipientEmail = companyEmail || email; // fallback to form email if companyEmail missing

    if (recipientEmail) {
      const ticketId = report.jiraIssueKey;
      const ackMailOptions = {
        from: `"Cloud Sentrics Support" <${process.env.EMAIL_USER}>`,
        to: recipientEmail, // ✅ send to company email
