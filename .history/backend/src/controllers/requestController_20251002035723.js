...
exports.createRequest = async (req, res) => {
  try {
    const { title, reporterEmail, awsAccounts, acknowledgements, selectedStorageCount } = req.body;
    ...
    const newRequest = new Request({
      ...
      awsAccounts,
      acknowledgements,
      selectedStorageCount, // save count in DB
    });
    ...
    // Prepend number of accounts if provided
    if (selectedStorageCount && selectedStorageCount > 0) {
      awsContent.unshift({
        type: "paragraph",
        content: [
          { type: "text", text: `Number of AWS Accounts Needed: ${selectedStorageCount}` },
        ],
      });
    }
    ...
    // 4️⃣ Save Jira Issue Key back to MongoDB
