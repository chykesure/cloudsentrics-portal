...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const resetUrl = `${process.env.FRONTEND_URL || "https://onboardingportal.cloudsentrics.org"}/admin/reset-password/${resetToken}`;

    const html = `
  <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f6f8; padding: 40px 0; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 6px 20px rgba(0,0,0,0.1);">
      
      <!-- Header -->
      <div style="background-color: #004aad; color: #ffffff; text-align: center; padding: 25px 40px;">
        <h1 style="margin: 0; font-size: 24px; font-weight: 700;">CloudSentric Portal</h1>
      </div>

      <!-- Body -->
      <div style="padding: 30px 40px;">
        <p style="font-size: 16px; margin-bottom: 20px;">
          Hello <strong>${staff.email}</strong>,
        </p>
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="${resetUrl}" 
             style="display: inline-block; background-color: #004aad; color: #ffffff; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-size: 16px; font-weight: 600; transition: background-color 0.3s;">
             Reset Password
          </a>
        </div>

        <p style="font-size: 14px; color: #555; line-height: 1.5;">
          If you didn’t request this, you can safely ignore this email. This link expires in 10 minutes.
        </p>
      </div>

      <!-- Footer -->
      <div style="background-color: #f9fafb; padding: 20px 40px; text-align: center; font-size: 12px; color: #999;">
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    // ✅ Assign trimmed password directly; pre-save hook will hash it
