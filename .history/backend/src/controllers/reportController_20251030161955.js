...
exports.createReport = async (req, res) => {
  try {
    ...
    // ✅ Save report in MongoDB first
    const report = new Report({ ...cleanData, image: imageData });
    await report.save();
    ...
    const descriptionADF = {
      ...
      version: 1,
      content: [
        { type: "paragraph", content: [{ type: "text", text: `Reporter: ${cleanData.fullName} (${cleanData.email})` }] },
    ...
    const jiraResponse = await axios.post(
      `${process.env.JIRA_BASE_URL}/rest/api/3/issue`,
      {
        fields: {
          ...
          summary: cleanData.title || `Issue Report from ${cleanData.email}`,
