...
exports.handleStatusUpdate = async (req, res) => {
  ...
  const db = mongoClient.db("cloudsentrics");
  const onboardingCollection = db.collection("onboardings");
  ...
  // ------------------------------
  // Template map (updated to use {{companyName}})
  // ------------------------------
  const templates = {
    request: {
      "In Progress": {
...
www.cloudsentrics.org`,
      ...
      Done: {
        subject: "Your Issue Has Been Completed – [Ticket #{{ticket_id}}]",
        body: `Dear {{companyName}},
We’re pleased to inform you that your issue (Ticket ID: {{ticket_id}}) has been successfully Completed.
Please reach out to us for any questions or concerns regarding this ticket.
Thank you for choosing Cloud Sentrics Limited, we appreciate your trust in our services.
Best regards,
...
www.cloudsentrics.org`,
  ...
  try {
    ...
    // 🔥 Fetch companyName from onboardings collection using reporter email
    ...
    try {
      if (reporterEmail) {
        ...
        const onboardingRecord = await onboardingCollection.findOne({
          "companyInfo.companyEmail": reporterEmail
        });

        if (onboardingRecord?.companyInfo?.companyName) {
          companyName = onboardingRecord.companyInfo.companyName.trim();
        } else {
          // Optional: try Jira custom fields (if your Jira issue has company info)
          const jiraCompany = payload.issue.fields.customfield_10050; // example
          if (jiraCompany) companyName = jiraCompany.trim();
        }
      }
    } catch (err) {
      console.error("⚠️ Error fetching company name:", err);
    }
