...
exports.getDashboard = async (req, res) => {
  try {
    ...
    if (!companyEmail) {
      return res.status(400).json({ message: "Missing companyEmail in request" });
    }

    // 1️⃣ Fetch user’s requests from MongoDB
    const userRequests = await Request.find({ companyEmail });
    ...
    // 2️⃣ Fetch all Jira issues
    const jiraIssues = await fetchAllJiraIssues();

    // 3️⃣ Match Jira issues to this user's requests
    const userJiraIds = userRequests.map(req => req.jiraIssueId).filter(Boolean);
    const userIssues = jiraIssues.filter(issue => userJiraIds.includes(issue.id));

    // 4️⃣ Map cleanly for frontend
    ...
    console.log(`✅ Dashboard fetched for ${companyEmail}: ${issues.length} issues`);
