...
exports.assignEngineer = async (req, res) => {
  try {
    ...
    // 2️⃣ Update assigned engineer
    request.assignedEngineer = engineerEmail;
    request.status = "Assigned"; // optional
    await request.save();

    // 3️⃣ Send notification to reporter
    if (request.reporterEmail) {
      ...
      const assignmentMail = {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: auto; color: #333;">
            <p>Dear Client,</p>
            <p>Your request (Ticket ID: <strong>${ticketId}</strong>) has now been assigned to an engineer: <strong>${engineerEmail}</strong>.</p>
            <p>They will review your case and provide updates accordingly.</p>
            <br/>
            <p>Kind regards,</p>
            <p><strong>Cloud Sentrics Support Team</strong><br/>
            Cloud Sentrics Limited<br/>
      ...
      sendEmail(assignmentMail)
        .then(info => console.log("Assignment email sent:", info.response))
        .catch(err => console.error("Error sending assignment email:", err));
    }

    res.status(200).json({ message: "Engineer assigned and reporter notified", request });
  } catch (error) {
    console.error("❌ Error assigning engineer:", error.message);
