...
exports.createRequest = async (req, res) => {
  try {
    ...
    // 1️⃣ Filter out empty or undefined values (clean payload)
    ...
    // 2️⃣ Tier mapping with fallback
    const tierMap = {
      standard: {
        title: "STANDARD TIER",
        storage: "200GB",
    ...
    const selectedTier =
      cleanData.selectedTier?.toLowerCase() || "standard";
    const tierDetails =
      tierMap[selectedTier] || {
        title: `CUSTOM TIER (${cleanData.selectedTier})`,
        storage: cleanData.selectedTier,
        channels: ["Dashboard"],
        response: "Varies",
    ...
    // 3️⃣ Save only filled data to MongoDB
    ...
    // 4️⃣ Build Jira ADF description dynamically
