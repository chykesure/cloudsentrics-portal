...
exports.handleStatusUpdate = async (req, res) => {
  ...
  try {
    ...
    if (payload.issue.fields.reporter) {
      console.log("\x1b[34m[Reporter Found]\x1b[0m", payload.issue.fields.reporter);
      ...
      console.log("\x1b[33m⚠️ No reporter field in payload.issue.fields\x1b[0m");
    }

    // If reporter not available, fallback to creator
    if (!reporterEmail && payload.issue.fields.creator) {
      console.log("\x1b[36m[Fallback to Creator]\x1b[0m", payload.issue.fields.creator);
    ...
    if (!reporterEmail) {
      console.warn(
        `\x1b[31m❌ Reporter email is NULL for issue ${issueKey}\x1b[0m`
      );
      console.log(
        "\x1b[33mPayload summary:\x1b[0m",
        JSON.stringify(
          {
            issueKey,
            reporter: payload.issue.fields.reporter,
            creator: payload.issue.fields.creator,
          },
          null,
          2
        )
    ...
    console.log(`\x1b[32m✅ Reporter email resolved:\x1b[0m ${reporterEmail}`);
