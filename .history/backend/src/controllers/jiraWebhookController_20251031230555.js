...
exports.handleStatusUpdate = async (req, res) => {
  ...
  try {
    ...
    // 5️⃣ If still not found, fetch from Jira API
    ...
    // 6️⃣ Cleanup email (avoid "No recipients defined")
    if (reporterEmail) {
      reporterEmail = reporterEmail.replace(/[<>]/g, "").trim();
    }

    // Optional: route reports to internal team email
    const recipient = isReport
      ? process.env.REPORT_TEAM_EMAIL || "reports@cloudsentrics.org"
      : reporterEmail;
    ...
    // 8️⃣ Send the email asynchronously
    setImmediate(async () => {
      try {
        await sendEmail({
        ...
        console.log(`✅ ${issueType} update email sent for ${issueKey}: ${oldStatus} → ${newStatus}`);
      } catch (mailErr) {
