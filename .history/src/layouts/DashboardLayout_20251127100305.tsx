...
const DashboardLayout = () => {
  ...
  const navItems = [
    ...
    { to: "/dashboard/upgrade-tier", label: "Upgrade Storage Tier", icon: ArrowUpCircle },
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      // 🔥 ensure companyEmail exists
      if (!parsed.companyEmail && parsed.email) {
        parsed.companyEmail = parsed.email;
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      if (!token || !parsedUser?.companyEmail) {
        localStorage.clear();
        navigate("/login");
        return;
      }
      ...
      try {
        ...
        const fullUser = {
          ...profile,
          companyName:
            companyData?.companyInfo?.companyName?.trim() ||
            parsedUser.companyEmail ||
            "Unknown Company",
        };
