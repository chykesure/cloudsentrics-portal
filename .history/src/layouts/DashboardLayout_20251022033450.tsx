...
const DashboardLayout = () => {
  ...
  // ✅ Load user on mount and fetch full company details
  useEffect(() => {
    const fetchCompanyDetails = async () => {
      ...
      setUserLoaded(false); // start loading

      const { companyEmail } = JSON.parse(storedUser);

      try {
        const res = await fetch(
          `https://api.onboardingportal.cloudsentrics.org/api/onboarding/company?email=${companyEmail}`,
          {
            method: "GET",
            headers: {
        ...
        if (!company) {
          console.warn("Company data not found, keeping previous user data.");
          ...
          const avatarUrl = company.avatar
            ? company.avatar.startsWith("http")
              ? company.avatar
              : `https://api.onboardingportal.cloudsentrics.org${company.avatar}`
        ...
        // don't log out immediately, keep localStorage user
        setUser(JSON.parse(storedUser));
      } finally {
