...
const DashboardLayout = () => {
  ...
  // ✅ Fetch full profile and companyName
  // Use state instead of localStorage
  useEffect(() => {
    if (!user) return; // wait until user is loaded
    const fetchUserProfile = async () => {
      const token = localStorage.getItem("token");
      if (!token || !user.companyEmail) {
        localStorage.clear();
        navigate("/login");
        return;
      }
      try {
        ...
        const companyRes = await fetch(
          `https://api.onboardingportal.cloudsentrics.org/api/onboarding/company?email=${user.companyEmail}`,
        ...
        const fullUser = {
          ...profileData.data,
          companyName: companyData?.company?.companyName?.trim() || user.companyEmail || "Unknown Company",
        };
        setUser(fullUser);
        localStorage.setItem("user", JSON.stringify(fullUser));

        const avatarUrl = fullUser.avatar
          ? fullUser.avatar.startsWith("http")
            ? fullUser.avatar
            : `https://api.onboardingportal.cloudsentrics.org${fullUser.avatar}`
