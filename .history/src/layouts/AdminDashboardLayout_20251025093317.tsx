...
import { LogOut, LayoutDashboard, User, Menu, X, Clock } from "lucide-react";
  ...
  const fetchAdminProfile = async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        console.warn("Admin profile fetch returned no data");
      }
    } catch (err) {
      console.error("Error fetching admin profile:", err);
      // keep fallback avatar & admin email as null
    }
  };
  ...
  const Sidebar = () => (
    <div className="w-64 bg-[#032352] text-white flex flex-col shadow-xl h-full">
      ...
      <nav className="flex-1 px-3 py-6 space-y-2">
        {navItems
          .filter(({ label }) => {
            // Hide "Staff Management" if role is readonly
            if (admin?.role === "readonly" && label === "Staff Management") {
              return false;
            }
            return true;
          })
