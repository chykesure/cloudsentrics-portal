...
const OnboardingForm2: React.FC<OnboardingForm2Props> = () => {
  ...
  // ✅ Restore previous state if exists (for persistence when clicking Back)
  const savedStep1 = sessionStorage.getItem("formDataStep1");
  const savedAWS = sessionStorage.getItem("awsSetupData");
  const formDataStep1 = savedStep1 ? JSON.parse(savedStep1) : location.state?.formDataStep1;
  const savedSetup = savedAWS ? JSON.parse(savedAWS) : null;

  // ✅ Main state
  const [selectedAccounts, setSelectedAccounts] = useState<number | null>(
    savedSetup?.selectedAccounts ?? null
  );
  const [moreThanFive, setMoreThanFive] = useState<string>(
    savedSetup?.moreThanFive ?? ""
  );
  const [aliases, setAliases] = useState<{ [key: string]: string }>(
    savedSetup?.aliases ?? {}
  );
  const [bulkAliases, setBulkAliases] = useState<string>(
    savedSetup?.bulkAliases ?? ""
  );
  ...
  const handleBack = () => {
    ...
    sessionStorage.setItem("awsSetupData", JSON.stringify(awsSetupData));
    navigate("/signup"); // Back to step1 but keep data
  };
