...
interface OnboardingForm3Props {
  formDataStep1: OnboardingData["companyInfo"];
  userId?: string;
}

const OnboardingForm3: React.FC<OnboardingForm3Props> = ({ formDataStep1, userId }) => {
  const navigate = useNavigate();
  const location = useLocation();

  // Get awsSetupData from location.state
  const { awsSetupData } = location.state as { awsSetupData: OnboardingData["awsSetup"] };
  ...
  const handleSubmit = () => {
    if (isSubmitDisabled) {
      toast.error("Please check all the boxes before submitting.", { position: "top-right" });
      return;
    }
  ...
  return (
    <div className="relative flex flex-col md:flex-row min-h-screen w-full overflow-hidden">
      ...
      >
        {/* Logo */}
        <img src={logo} alt="Cloud Sentrics" className="mb-2 h-12 sm:h-36 object-contain" />
        ...
        <div className="space-y-6 sm:space-y-8 text-base sm:text-lg leading-relaxed">
          <label className="flex items-start space-x-3 cursor-pointer">
            ...
            />
            <span>
              <strong>I agree</strong> that the selections above are accurate and I am authorized to submit this onboarding on behalf of my organization.
            </span>
          </label>
          ...
          <label className="flex items-start space-x-3 cursor-pointer">
            ...
            />
            <span>
              <strong>I acknowledge</strong> that Cloud Sentrics may store my data in AWS regions outside of my country or geographic region, and that such storage will still comply with all applicable data protection regulations.
            </span>
          </label>
          ...
          <label className="flex items-start space-x-3 cursor-pointer">
            ...
            />
            <span>
              <strong>I confirm</strong> that I will not upload, store, or process any prohibited data on Cloud Sentrics' systems.
            </span>
          </label>
        </div>
      ...
      <motion.div
        ...
        animate={{ x: 0, opacity: 1 }}
        transition={{ duration: 0.8 }}
        className="relative flex-1 items-center justify-center overflow-hidden h-48 md:h-auto md:flex"
      >
        ...
        <div className="relative text-center text-white max-w-lg p-6 sm:p-12">
          ...
          <p className="mt-2 sm:mt-4 italic text-sm sm:text-lg md:text-xl">
            Please review each section carefully and ensure all details match your intended setup.
          </p>
        </div>
      </motion.div>
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          ...
          >
            ...
            <p className="text-gray-700 font-medium mb-2">Account Created Successfully</p>
            <p className="text-gray-600 mb-6">Check your email for your customer ID</p>
