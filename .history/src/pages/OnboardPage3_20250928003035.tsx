...
const OnboardingForm3: React.FC<OnboardingForm3Props> = ({ formDataStep1 }) => {
  const navigate = useNavigate();
  const location = useLocation();

  // Retrieve AWS setup data from previous step
  const awsSetupData = (location.state as { awsSetupData?: OnboardingData["awsSetup"] })?.awsSetupData;
  const formDataStep11 = (location.state as { formDataStep1?: OnboardingData["companyInfo"] })?.formDataStep1;

  // Redirect back if awsSetupData is missing
  useEffect(() => {
    if (!awsSetupData) {
  ...
  const handleSubmit = async () => {
    ...
    if (!awsSetupData) return; // extra safety

    const payload = {
      companyInfo: awsSetupData,
