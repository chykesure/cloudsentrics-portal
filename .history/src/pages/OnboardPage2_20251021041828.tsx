...
const OnboardingForm2: React.FC<OnboardingForm2Props> = () => {
  ...
  // When user changes number of accounts, reset aliases
  useEffect(() => {
    const count =
      selectedAccounts || (moreThanFive ? Number(moreThanFive) : 0);
    const newAliases: { [key: string]: string } = {};
    for (let i = 0; i < count; i++) {
      newAliases[String.fromCharCode(65 + i)] = "";
    }
  ...
  // Determine how many alias fields should appear
  const aliasCount =
    selectedAccounts || (moreThanFive ? Number(moreThanFive) : 0);

  const aliasKeys = Array.from({ length: aliasCount }, (_, i) =>
    String.fromCharCode(65 + i)
  );

  // Disable Next button until all alias fields are filled
  const filledAliasesCount = aliasKeys.filter(
    (key) => aliases[key]?.trim() !== ""
  ).length;

  const isNextDisabled =
    aliasCount === 0 || filledAliasesCount < aliasCount;
  ...
  return (
    <div className="relative flex flex-col md:flex-row min-h-screen w-full overflow-hidden">
      ...
      >
        ...
        <div className="mb-6 sm:mb-8">
          ...
          {aliasCount > 0 ? (
            ...
            <p className="text-gray-500 italic text-sm">
              Select or enter number of accounts to show input fields.
            </p>
          )}
        </div>
        ...
        <div className="mb-6 sm:mb-8">
          <label className="block text-sm sm:text-lg font-semibold mb-2">
            Enter more AWS Aliases if more than selected number
          </label>
          <textarea
            value={extraAliases}
      ...
      >
        ...
        <div className="relative text-center text-white max-w-lg p-4 sm:p-8 md:p-12">
          ...
          <p className="text-xs sm:text-base md:text-xl leading-relaxed mb-2 sm:mb-6">
            This form captures the confirmed details of your Cloud Sentrics
            service so we can provision your AWS environment accurately.
          </p>
