...
const OnboardingForm3: React.FC = () => {
  ...
  // ✅ Updated agreements state (includes signedBy)
  ...
  const handleCheckboxChange = (key: keyof typeof agreements) => {
    if (key === "signedBy") return; // skip for text input
    setAgreements((prev) => ({ ...prev, [key]: !prev[key] }));
  };

  // ✅ Updated validation: must also include signedBy name
  const isSubmitDisabled =
    !agreements.agree ||
    !agreements.acknowledge ||
    !agreements.confirm ||
    !agreements.signedBy.trim();

  const handleSubmit = async () => {
    if (isSubmitDisabled) {
      toast.error("Please complete all agreements and sign before submitting.", {
        position: "top-right",
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error("Duplicate entry Company Email is already registered.", {
          position: "top-right",
        });
      }
    } catch (err) {
        ...
        <div className="space-y-6 sm:space-y-8 text-base sm:text-lg leading-relaxed">
          ...
          <div className="mt-6">
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded-lg px-4 py-2 text-base sm:text-lg focus:outline-none focus:ring-2 focus:ring-blue-700"
            />
          </div>
        </div>
          ...
          <motion.button
            ...
            onClick={() =>
              navigate("/signup/step2", {
                state: { formDataStep1, awsSetupData },
              })
            }
