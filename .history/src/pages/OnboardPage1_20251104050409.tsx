...
const OnboardingForm1: React.FC = () => {
  ...
  const storedData = sessionStorage.getItem("onboardingStep1");
  const initialData: FormData = storedData
  ...
  // ✅ Save automatically to sessionStorage when formData changes
  useEffect(() => {
 ...
 const handleNext = () => {
  // Jump directly to step 3, skipping step 2
  navigate("/signup/step3", { 
    state: { 
      formDataStep1: formData,
      awsSetupData: {} // provide empty object to satisfy step3 check
    } 
  });
};
    ...
    <div className="relative flex flex-col md:flex-row min-h-screen w-full overflow-hidden">
      ...
      >
        ...
        <button
          ...
          className={`w-full rounded-lg py-3 font-semibold shadow text-sm sm:text-base transition ${
            isNextDisabled
