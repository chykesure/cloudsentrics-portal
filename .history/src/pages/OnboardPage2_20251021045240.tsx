...
const OnboardingForm2: React.FC<OnboardingForm2Props> = () => {
  ...
  // 🧠 Try to load from navigation or sessionStorage
  /*   const formDataStep1 =
      (location.state as { formDataStep1?: OnboardingData["companyInfo"] })?.formDataStep1 ||
      JSON.parse(sessionStorage.getItem("formDataStep1") || "null"); */
  ...
  const awsSetupData = savedAWS ? JSON.parse(savedAWS) : location.state?.awsSetupData;


  // If still not found, redirect to step1
  useEffect(() => {
    if (!formDataStep1) {
  ...
  const awsSetupData =
    savedAWS
      ? JSON.parse(savedAWS)
      : (location.state as { awsSetupData?: OnboardingData["awsSetup"] })?.awsSetupData;
      ...
      >
        ...
        {isHybridMode && (
          <div className="mb-6 sm:mb-8">
            ...
            <textarea
              ...
              rows={5}
              className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 outline-none ${allSixFilled
                ? "focus:ring-blue-700"
        ...
        >
          ...
          <motion.button
            ...
            className={`px-6 py-2 rounded-lg font-semibold shadow transition ${isNextDisabled
