...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // ✅ Load current storage for CUSTOM tier directly from localStorage
    if (currentTierInfo?.title?.toUpperCase() === "CUSTOM") {
      try {
        const backendData =
          JSON.parse(localStorage.getItem("backendTierData") || "{}") || {};
        let storage = backendData.tierStorage || "50 GB"; // fallback

        // Update current tier info
        currentTierInfo = { ...currentTierInfo, storage };

        console.log("✅ Loaded custom tier from localStorage:", currentTierInfo);
      } catch (err) {
        console.warn("❌ Could not parse backend tier data from localStorage:", err);
        currentTierInfo = { ...currentTierInfo, storage: "50 GB" }; // safe fallback
      }
    }
