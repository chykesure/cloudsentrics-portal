...
const handleSelectTier = async (tierId: string) => {
  ...
  // -------------------------------
  // Get user and current tier info
  // -------------------------------
  const user = JSON.parse(localStorage.getItem("user") || "{}");
  ...
  if (!userId) {
    Swal.fire({
      ...
      text: "Cannot submit request without a valid user ID.",
      confirmButtonColor: "#032352",
    });
    return;
  }
  ...
  if (currentTierIdResolved === "custom") {
    try {
      ...
      currentTierInfo = null; // first-time if fetch fails
    }
  }
