...
const DashboardHome = () => {
  ...
  const darkBlue = "#032352"; // primary dark blue
  const lighterBlue = "#0b3b6a"; // slightly lighter for gradients

  const stats = [
    { title: "Profile", icon: User },
    { title: "Request Portal", icon: FileText },
    ...
    <div className="min-h-screen bg-white p-6 sm:p-10 space-y-10">
      ...
      <div className="relative w-full">
        <motion.div
          ...
          className={`bg-gradient-to-r from-[${darkBlue}] to-[${lighterBlue}] text-white rounded-2xl shadow-lg p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10`}
        >
        ...
        {stats.map((stat, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`bg-gradient-to-r from-[${darkBlue}] to-[${lighterBlue}] text-white rounded-2xl shadow-md p-6 flex items-center gap-4 cursor-pointer transition`}
            >
        ...
        {actions.map((action, idx) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{ scale: 1.03 }}
              className={`bg-gradient-to-r from-[${darkBlue}] to-[${lighterBlue}] text-white rounded-2xl shadow-md p-6 flex flex-col items-start gap-4 hover:shadow-lg transition cursor-pointer`}
            >
      ...
      {/* Announcements */}
      <div className={`bg-gradient-to-r from-[${darkBlue}] to-[${lighterBlue}] rounded-2xl shadow-md p-6`}>
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2 text-white">
          <Star className="w-5 h-5 text-white" /> Recent Announcements
        </h2>
        <div className="flex flex-col gap-4">
          {announcements.map((ann) => (
            <motion.div
              ...
              className={`border-l-4 border-white bg-[${darkBlue}] p-4 rounded-lg cursor-pointer transition`}
            >
      ...
      {tipOpen && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className={`bg-gradient-to-r from-[${darkBlue}] to-[${lighterBlue}] text-white rounded-2xl p-6 shadow-md flex flex-col md:flex-row items-center justify-between gap-4`}
        >
          ...
          <button
            ...
            className="px-6 py-2 bg-white text-[${darkBlue}] rounded-lg font-semibold hover:bg-gray-100 transition"
          >
            Dismiss
          </button>
        </motion.div>
