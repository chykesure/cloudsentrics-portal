...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    const [backendTierData, setBackendTierData] = useState<ReportFormData | null>(null);

// After fetching:
setBackendTierData(response.data);
    ...
    if (tierId === "custom") {
      ...
      // If CUSTOM, prefer backend value
      if (currentTier === "CUSTOM" && (!displayCurrentStorage || displayCurrentStorage === "0GB" || displayCurrentStorage === "Custom Storage")) {
        displayCurrentStorage =
          backendTierData?.tierStorage || // ✅ backend storage
          currentTierInfo?.storage ||      // fallback to currentTierInfo
          "Custom Storage";                // last resort
      }
