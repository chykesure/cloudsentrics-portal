...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // --- Add support for CUSTOM tier behaving like standard tiers
    if (!currentTierInfo && currentTier === "CUSTOM") {
      const user = JSON.parse(localStorage.getItem("user") || "{}");
      currentTierInfo = {
        id: "custom",
        title: "CUSTOM",
        storage: user.tierStorage || "0GB", // use actual storage from user
        rank: 999, // optional: treat CUSTOM as highest rank
        channels: [],
        response: "",
        availability: "",
        extras: "",
    ...
    if (tierId === "custom") {
      ...
      const confirmUpgrade = await Swal.fire({
        ...
        html: `You are currently on <b>${currentTierInfo?.title || "CUSTOM"}</b> (${currentTierInfo?.storage || "0GB"
          }) and about to upgrade to <b>CUSTOM</b> (${customInput.amount} ${customInput.unit
