...
const DashboardHome = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 sm:p-10 space-y-10">
        ...
        {stats.map((stat, idx) => {
          ...
          return (
            <motion.div
              ...
              className="bg-gradient-to-r from-[#032352] to-[#0b3b6a] text-white rounded-2xl shadow-lg p-6 flex items-center gap-4 cursor-pointer transition"
            >
        ...
        {actions.map((action, idx) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{ scale: 1.05 }}
              className="bg-gradient-to-r from-[#032352] to-[#0b3b6a] text-white rounded-2xl shadow-lg p-6 flex flex-col items-start gap-4 hover:shadow-2xl transition cursor-pointer"
            >
      ...
      {/* Announcements */}
      <div className="bg-gradient-to-r from-[#032352]/20 to-[#0b3b6a]/20 rounded-2xl shadow-lg p-6">
        ...
        <div className="flex flex-col gap-4">
          {announcements.map((ann) => (
            <motion.div
              ...
              className="border-l-4 border-white/60 bg-white/10 p-4 rounded-lg cursor-pointer transition"
            >
              <div className="flex justify-between items-center">
                <h3 className="font-semibold text-white">{ann.title}</h3>
                <span className="text-white/70 text-sm">{new Date(ann.date).toLocaleDateString()}</span>
              </div>
              <p className="text-white/70 mt-1">{ann.message}</p>
            </motion.div>
          ))}
        </div>
      </div>
