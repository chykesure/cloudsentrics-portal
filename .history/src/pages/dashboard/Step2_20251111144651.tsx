...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (tierId === "custom") {
      ...
      // If it's CUSTOM but not set properly, try checking the user data or formData
      if (currentTier === "CUSTOM" && (!displayCurrentStorage || displayCurrentStorage === "0GB")) {
  const backendData = JSON.parse(localStorage.getItem("backendTierData") || "{}");
  const user = JSON.parse(localStorage.getItem("user") || "{}");

  displayCurrentStorage =
    backendData.tierStorage ||   // ✅ take real value from backend
    user.tierStorage ||          // fallback to user data
    formData?.tierDetails?.storage ||
    "Custom Storage";            // last resort
}
