...
const Step6 = ({ goBack, onSubmit, formData }: StepProps) => {
  ...
  // ✅ Submit full request (Step1 → Step6)
    ...
    try {
      ...
      const descriptionADF = {
        ...
        content: [
          ...
          // ✅ Include Change data if exists
          ...(formData.existingAccountId || formData.existingStorageName || (formData.changesRequested?.length ?? 0) > 0
            ? [
                { type: "paragraph", content: [{ type: "text", text: `Change Request Details:` }] },
                ...(formData.existingAccountId ? [{ type: "paragraph", content: [{ type: "text", text: `- Existing Account ID: ${formData.existingAccountId}` }] }] : []),
                ...(formData.existingStorageName ? [{ type: "paragraph", content: [{ type: "text", text: `- Existing Storage Name: ${formData.existingStorageName}` }] }] : []),
                ...(formData.changesRequested?.map(req => ({ type: "paragraph", content: [{ type: "text", text: `- Change: ${req}` }] })) ?? []),
                ...(formData.details ? [{ type: "paragraph", content: [{ type: "text", text: `- Additional Details: ${formData.details}` }] }] : []),
              ]
            : []
          ),
