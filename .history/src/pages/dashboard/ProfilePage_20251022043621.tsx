...
const ProfilePage = () => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        setForm({
          ...
          phone: profile.phone || "",
          // ✅ only use tier from DB — no default tier
          tier: profile.tier || "",
  ...
  const renderTierBadge = (tier: string) => {
    // 🛡️ Strong guard: hide badge for null, undefined, empty, "null", "undefined", or whitespace
    if (!tier || tier.trim() === "" || tier.toLowerCase() === "null" || tier.toLowerCase() === "undefined") {
      return null;
    }
    ...
    const tierStyle = styleMap[normalizedTier] || styleMap["STANDARD TIER"];
