...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        if (data.avatar) setAvatar(data.avatar.startsWith("http") ? data.avatar : `http://localhost:5000${data.avatar}`);
      } catch (err: any) {
  ...
  // Handle avatar upload + save to localStorage
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // ✅ If backend avatar exists, save full path to localStorage
