...
const Step1 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
      ...
      {selectedOption === "aws" && (
        <div className="w-full px-2 sm:px-6 mb-12">
              ...
              <div key={label}>
                ...
                <div className="flex flex-col sm:flex-row">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAwsAliases((prev) => ({ ...prev, [label]: e.target.value }))
                    }
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 mt-10">
            ...
            <button
  onClick={() => {
    // Ensure selectedStorageCount is a number
    const awsCount = selectedStorageCount ?? 0;

    // Prepare descriptive text for Jira
    const awsCountText =
      awsCount > 0 ? `${awsCount} AWS Account(s) requested` : "No AWS accounts requested";

    // Save AWS data into formData
    setFormData({
      ...formData,
      selectedStorageCount: awsCount,
      awsAliases: Object.values(awsAliases).filter(alias => alias.trim() !== ""),
      bucketNote,
      awsCountText,
    });
  ...
  disabled={
    ...
    !(
      Object.values(awsAliases).some((alias) => alias.trim() !== "") ||
      bucketNote.trim() !== ""
    )
  }
