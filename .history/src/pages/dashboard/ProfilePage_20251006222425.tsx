...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfileAndRequest = async () => {
      try {
        const profileRes = await axios.get(`http://localhost:5000/api/profile/${email}`);
        ...
        try {
          const requestsRes = await axios.get(`http://localhost:5000/api/requests?reporterEmail=${email}`);
          const latestRequest = requestsRes.data?.[0];
          if (latestRequest) {
            tier = latestRequest.selectedTier || latestRequest.tier || "";
            storage = (latestRequest.selectedStorageCount ?? latestRequest.storageCount ?? "").toString();
          }
        } catch (err) {
          console.warn("No requests found or endpoint missing, skipping tier/storage", err);
        }

        const profileData = { ...profileRes.data, tier, storage };
        setForm(profileData);
        setAvatar(profileData.avatar || defaultAvatar);
      } catch (err) {
        console.error("Error fetching profile or requests:", err);
        setError("Failed to fetch profile or request data.");
      } finally {
        setLoading(false);
      }
    };

    fetchProfileAndRequest();
  }, []);
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 text-gray-700">
            <div className="flex flex-col">
              <p className="font-semibold text-xs sm:text-sm text-[#032352]/70">NAME</p>
              <p className="text-base sm:text-lg font-medium text-[#032352]">{form.name}</p>
            </div>
            <div className="flex flex-col">
              <p className="font-semibold text-xs sm:text-sm text-[#032352]/70">EMAIL</p>
              <p className="text-base sm:text-lg font-medium text-[#032352]">{form.companyEmail}</p>
            </div>
            <div className="flex flex-col">
              <p className="font-semibold text-xs sm:text-sm text-[#032352]/70">PHONE</p>
              <p className="text-base sm:text-lg font-medium text-[#032352]">{form.phone}</p>
            </div>
            <div className="flex flex-col">
              <p className="font-semibold text-xs sm:text-sm text-[#032352]/70">CUSTOMER ID</p>
              <p className="text-base sm:text-lg font-medium text-[#032352]">{form.customerId}</p>
            </div>
          </div>
        </motion.div>
