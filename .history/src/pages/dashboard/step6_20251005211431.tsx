...
const Step6 = ({ goBack, onSubmit, formData }: StepProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Build Jira description (ADF) with AWS, Storage, Change
      const descriptionADF = {
        ...
        content: [
          ...
          // Storage info
          formData.step4Data ? { type: "paragraph", content: [{ type: "text", text: `Storage Settings: ${JSON.stringify(formData.step4Data)}` }] } : null,
          // Change info
          ...(formData.changesRequested?.length
            ? [
                { type: "paragraph", content: [{ type: "text", text: `Change Requests:` }] },
                ...formData.changesRequested.map((c: string) => ({
                  type: "paragraph",
      ...
      // Combine all step data into payload
      const payload = {
