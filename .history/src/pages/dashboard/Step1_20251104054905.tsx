...
const Step1 = ({ goBack, jumpToStep, setFormData }: StepProps) => {
  ...
  const confirmBeforeNext = async (callback: () => void) => {
    const result = await Swal.fire({
      title: "Confirm Information",
      text: "Are you sure all information entered is correct before proceeding?",
      icon: "question",
  ...
  return (
    ...
    >
      ...
      {/* ==================== AWS SECTION ==================== */}
      {selectedOption === "aws" && (
        <div className="w-full px-2 sm:px-6 mb-12 transition-all duration-300">
          {/* Count Selection */}
          <div className="flex flex-col sm:flex-row sm:items-center mb-8 gap-4 text-base sm:text-xl">
            <label className="font-medium">Number of AWS Accounts Needed</label>
              ...
              {[1, 2, 3, 4, 5].map((n) => (
                <label key={n} className="flex items-center relative cursor-pointer">
                  <input
                    ...
                    name="awsCount"
                    value={n}
                    checked={selectedAWSCount === n}
                    onChange={() => {
                      setSelectedAWSCount(n);
                      setFormData((prev) => ({ ...prev, selectedAWSCount: n }));
                    }}
                  ...
                  />
                  <span
                    className={`pointer-events-none absolute left-1 top-0.5 w-5 h-5 flex items-center justify-center text-white ${selectedAWSCount === n ? "opacity-100" : "opacity-0"
            ...
            <div className="flex flex-col w-full sm:w-64">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setSelectedAWSCount(count);
                  setFormData((prev) => ({ ...prev, selectedAWSCount: count }));
                }}
                value={selectedAWSCount ?? ""}
              />
            </div>
          </div>

          {/* AWS Aliases */}
          {selectedAWSCount && selectedAWSCount > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10 transition-all duration-500 ease-in-out">
              {Array.from({ length: Math.min(selectedAWSCount, 6) }).map((_, index) => {
                  ...
                  <div key={label}>
                    ...
                    <div className="flex flex-col sm:flex-row">
                      ...
                      <input
                        ...
                        className="flex-1 px-4 py-3 border border-gray-300 rounded-b-md sm:rounded-r-md text-sm sm:text-lg bg-white text-gray-800"
                      />
                    </div>
                  </div>
                );
          ...
          {/* Extra Textarea */}
          {selectedAWSCount && selectedAWSCount > 6 && (
            <div className="mb-10">
              ...
              <textarea
                ...
                onChange={(e) => {
                  const val = e.target.value;
                  setAWSNote(val);
                  setFormData((prev) => ({ ...prev, awsNote: val }));
                }}
                placeholder="Starting with cloudsentrics-aws-"
                className="w-full p-4 border border-gray-300 rounded-md text-sm sm:text-lg resize-none bg-white text-gray-800"
                maxLength={maxLength}
              ></textarea>
              <div className="text-right text-sm sm:text-md text-gray-500 mt-1">
                {awsNote.length}/{maxLength}
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 mt-10">
            ...
            <button
              onClick={() => confirmBeforeNext(() => jumpToStep?.(6))}
              disabled={
                !selectedOption ||
                selectedOption !== "aws" ||
                !selectedAWSCount ||
                (selectedAWSCount <= 6 &&
                  Array.from({ length: selectedAWSCount }).some(
                    (_, i) => !awsAliases[String.fromCharCode(65 + i)]?.trim()
                  )) ||
                (selectedAWSCount > 6 &&
                  (["A", "B", "C", "D", "E", "F"].some((l) => !awsAliases[l]?.trim()) ||
                    !awsNote.trim()))
              }
      ...
      {selectedOption === "storage" && (
        <div className="mb-12 transition-all duration-300">
          ...
          <div className="flex flex-wrap items-center mb-8 gap-4 text-base sm:text-xl">
            {[1, 2, 3, 4, 5].map((n) => (
              <label key={n} className="flex items-center space-x-2 cursor-pointer relative">
                <input
                  type="radio"
                  name="storageCount"
                  value={n}
                ...
                />
                <span
                  className={`absolute left-1 top-0.5 w-5 h-5 flex items-center justify-center text-white ${selectedStorageCount === n ? "opacity-100" : "opacity-0"
                    } transition-opacity duration-200`}
                >
                  ✓
                </span>
          ...
          {/* Storage Inputs */}
          {selectedStorageCount && selectedStorageCount > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10 transition-all duration-500 ease-in-out">
              {Array.from({ length: Math.min(selectedStorageCount, 6) }).map((_, index) => {
                const label = String.fromCharCode(65 + index);
                return (
                  <div key={label}>
                    <label className="text-base sm:text-xl font-medium text-gray-700 mb-2 block">
                    ...
                    <div className="flex flex-col sm:flex-row">
                      ...
                      <input
                        type="text"
                        placeholder="Organization-Purpose-CustomerID"
          ...
          {selectedStorageCount && selectedStorageCount > 6 && (
            <div className="mb-10">
              ...
              <textarea
                ...
                placeholder="cloudsentrics-storage-Organization-Purpose-CustomerID"
                className="w-full p-4 border border-gray-300 rounded-md text-sm sm:text-lg resize-none bg-white text-gray-800"
                maxLength={maxLength}
              ></textarea>
              <div className="text-right text-sm sm:text-md text-gray-500 mt-1">
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 mt-10">
            ...
            <button
              onClick={() => selectedOption === "storage" && confirmBeforeNext(() => jumpToStep?.(2))}
              disabled={!isStorageFormValid()}
              className={`w-full sm:w-auto px-8 py-3 sm:px-10 sm:py-4 text-base sm:text-lg rounded-md ${!isStorageFormValid()
                  ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                  : "bg-[#032352] text-white hover:bg-blue-700"
                }`}
            >
