  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (currentTierInfo?.title?.toUpperCase() === "CUSTOM") {
      try {
        ...
        const backendData = res.data?.data || {};
        let storage =
          backendData.tierStorage ||
            backendData.newStorage ||
            backendData.previousStorage ||
            currentTierInfo?.storage ||
            localStorage.getItem("backendTierData")
            ? JSON.parse(localStorage.getItem("backendTierData") || "{}").tierStorage
            : "50 GB"; // ✅ safe default

        if (!storage || storage === "0 GB") storage = "50 GB"; // final fallback
        ...
        console.log("✅ Loaded backend custom tier:", currentTierInfo);
      } catch (err) {
        console.warn("❌ Could not fetch backend custom tier data:", err);
        currentTierInfo = { ...currentTierInfo, storage: "50 GB" };
      }
    }
