...
const useCountUp = (end: number = 0, duration: number = 1500) => {
  ...
  useEffect(() => {
    const target = Number(end) || 0; // ensure it's a number
    let start = 0;
...
const DashboardHome = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("/api/dashboard"); // adjust to your endpoint
        const data = response.data || {};
        setDashboardData({
          totalRequests: Number(data.total) || 0,
          todo: Number(data.statuses?.["To Do"]) || 0,
          inProgress: Number(data.statuses?.["In Progress"]) || 0,
          inReview: Number(data.statuses?.["In Review"]) || 0,
          done: Number(data.statuses?.Done) || 0,
          issues: data.issues || [],
        });
      } catch (err) {
