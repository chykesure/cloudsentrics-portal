...
const Step1 = ({ goBack, jumpToStep, setFormData }: StepProps) => {
  ...
  // 🧠 Load persisted Step1 data on mount
useEffect(() => {
  const saved = localStorage.getItem("step1Data");
  if (saved) {
    const parsed = JSON.parse(saved);
    setSelectedOption(parsed.selectedOption || "");
    setSelectedStorageCount(parsed.selectedStorageCount || null);
    setAwsAliases(parsed.awsAliases || {});
    setBucketNote(parsed.bucketNote || "");
    setFormData((prev) => ({ ...prev, ...parsed }));
  }
}, []);

// 💾 Persist Step1 data whenever formData changes
useEffect(() => {
  const payload = {
    selectedOption,
  ...
  localStorage.setItem("step1Data", JSON.stringify(payload));
}, [selectedOption, selectedStorageCount, awsAliases, bucketNote]);
