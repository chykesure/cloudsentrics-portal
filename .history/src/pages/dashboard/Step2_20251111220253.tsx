...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (tierId === "custom") {
      ...
      if (tierId === "custom" && !currentTierInfo) {
        // First-time Custom selection → behave like standard tiers
        const endpoint = `${BASE_URL}/request`; // same as standard first-timer
  ...
  selectedTier: "custom",
  tierTitle: "CUSTOM",                  // <-- add this
  tierStorage: `${customInput.amount} ${customInput.unit}`,  // <-- add this
  tierDetails: {
    title: "CUSTOM",
  ...
  timestamp: new Date().toISOString(),
        ...
        await Swal.fire({
          icon: "success",
          title: "Tier Selected",
          text: `Your custom tier of ${customInput.amount} ${customInput.unit} has been saved.`,
          confirmButtonColor: "#032352",
        });
