...
const StatCard = ({ label, value, Icon, color }: StatCardProps) => {
  ...
  return (
    <motion.div
      ...
      className={`p-6 sm:p-8 min-h-[160px] rounded-2xl shadow-lg flex items-center gap-4 sm:gap-6 hover:shadow-2xl transition bg-white ${color}`}
    >
      ...
      <div>
        ...
        <p className="text-sm sm:text-base mt-2 opacity-90">{label}</p>
      </div>
    </motion.div>
  );
};
...
const DashboardHome = () => {
      ...
      try {
        ...
        setDashboardData({
          ...
          issues: (data.issues || []).map((issue: any) => ({
            ...
            created: issue.created || new Date().toISOString(),
  ...
  // Stats
  const stats = [
    { label: "Total Portal Requests", value: dashboardData.totalRequests, Icon: FileText, color: "text-[#032352]" },
