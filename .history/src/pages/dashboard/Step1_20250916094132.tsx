...
import { Mail, MessageCircle, Phone, MessageSquare, ChevronDown, X } from "lucide-react";
...
const RadioCheck = ({
  ...
  <label
    className={`flex items-center gap-2 cursor-pointer px-3 py-1 rounded-md border ${
      selected ? "bg-blue-600 text-white border-blue-600" : "border-gray-400"
    }`}
    onClick={onClick}
  >
...
const Step1 = ({ goNext, goBack }: StepProps) => {
  ...
  // Step3 states
  const [rows, setRows] = useState<AccessRow[]>([{ fullName: "", email: "", accessLevel: "" }]);
  const [setOpenDropdown] = useState<number | null>(null);
