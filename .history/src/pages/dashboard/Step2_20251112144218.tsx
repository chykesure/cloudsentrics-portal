...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // Resolve current tier from backend/localStorage
    let backendTierData: { tierStorage?: string } = {};
    try {
      const res = await axios.get(
        `${BASE_URL}/upgrade/request/by-email/${user.email || user.companyEmail}`
      );
      backendTierData = res.data.data || {};
    } catch (err) {
      console.warn("❌ Could not fetch backend tier data:", err);
    }

    // Resolve current tier info
    let currentTierIdResolved = resolveTierId(currentTier);
    let currentTierInfo: any = currentTierIdResolved
      ? tiers.find((t) => t.id === currentTierIdResolved) || null
      : null;

    // Override storage if backend has previous upgrades
    if (backendTierData.tierStorage) {
      currentTierInfo = {
        ...currentTierInfo,
        storage: backendTierData.tierStorage,
        title: currentTierIdResolved === "custom" ? "CUSTOM" : currentTierInfo?.title,
      };
    }
    ...
    if (!currentTierInfo) {
      ...
      if (tierId === "custom") {
        ...
        const { value: customInput } = await Swal.fire({
          ...
          preConfirm: () => {
            ...
            if (!amount || isNaN(amount)) Swal.showValidationMessage("Please enter a valid number");
            if (unit === "GB" && amount < 50) Swal.showValidationMessage("Minimum 50GB required");
            return { amount, unit };
          },
        });
        ...
        setFormData((prev) => ({
          ...prev,
          selectedTier: "custom",
          customAmount: customInput.amount,
          customUnit: customInput.unit,
          tierDetails: { title: "CUSTOM", storage: `${customInput.amount} ${customInput.unit}` },
        }));
        ...
        setFormData((prev) => ({
          ...
          tierDetails: {
            ...
            channels: selectedTier.channels.join(", "),
            response: selectedTier.response,
            availability: selectedTier.availability,
            extras: selectedTier.extras,
          },
        }));
      }
      ...
      await Swal.fire({
        icon: "success",
        title: "Tier Selected",
        text: `You’ve selected the ${tierId === "custom" ? "CUSTOM" : selectedTier?.title}.`,
        confirmButtonColor: "#032352",
      });
      return;
    }
    ...
    // -------------------------------
    // EXISTING USER FLOW (NORMAL & CUSTOM)
    // -------------------------------
    let selectedTierInfo = tiers.find((t) => t.id === tierId);

    if (tierId === "custom") {
      // Custom upgrade input
      const { value: customInput } = await Swal.fire({
        title: "Custom Tier Upgrade",
        html:
          '<input id="customAmount" type="number" class="swal2-input" placeholder="Enter amount">' +
          '<select id="customUnit" class="swal2-select">' +
          '<option value="GB">GB</option>' +
        ...
        preConfirm: () => {
          ...
          if (!amount || isNaN(amount)) Swal.showValidationMessage("Enter a valid number");
          if (unit === "GB" && amount < 50) Swal.showValidationMessage("Minimum 50GB required");
          return { amount, unit };
        },
      });
      ...
      const newStorageGB = customInput.unit === "TB" ? customInput.amount * 1024 : customInput.amount;

      if (newStorageGB < currentStorageGB) {
        await Swal.fire({
          icon: "warning",
          title: "Downgrade Not Allowed",
          text: `Your current storage is ${currentStorageGB} GB. Downgrades not allowed.`,
        });
        return;
      }
      ...
      const confirmUpgrade = await Swal.fire({
        ...
        html: `You are currently on <b>${currentTierInfo.title}</b> (${currentTierInfo.storage}) and upgrading to <b>CUSTOM</b> (${customInput.amount} ${customInput.unit}).`,
        icon: "question",
      ...
      try {
        ...
        console.error(err);
        Swal.fire({ icon: "error", title: "Upgrade Failed", text: err.message || "" });
      } finally {
        setLoading(false);
      }
    ...
    // NORMAL upgrade for existing tiers
    const currentStorageGB = parseStorageToGB(currentTierInfo.storage);
    ...
    if (selectedStorageGB < currentStorageGB) {
      await Swal.fire({
        icon: "warning",
        title: "Downgrade Not Allowed",
        text: `Your current storage is ${currentStorageGB} GB. Downgrades not permitted.`,
      });
      return;
    }
    ...
    const confirmUpgrade = await Swal.fire({
      title: "Confirm Upgrade",
      html: `You are currently on <b>${currentTierInfo.title}</b> (${currentTierInfo.storage}) and upgrading to <b>${selectedTierInfo.title}</b> (${selectedTierInfo.storage}).`,
      icon: "question",
      showCancelButton: true,
    });
