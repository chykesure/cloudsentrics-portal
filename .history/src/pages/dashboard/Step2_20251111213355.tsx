...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    try {
      ...
      // Always try fetching current backend tier (for Custom or active upgrades)
      const res = await axios.get(
        `${BASE_URL}/upgrade/request/by-email/${user.email || user.companyEmail}`
      );
      const backendData = res.data.data || null;

      if (backendData) {
        currentTierInfo = {
          id: resolveTierId(backendData.newTier) || "custom",
          title: backendData.newTier || "CUSTOM",
          storage: backendData.newStorage || "Custom Storage",
          rank: 999,
          channels: [],
          response: "",
          availability: "",
        ...
        console.log("📦 Existing tier found from backend:", currentTierInfo);
      } else {
        console.log("🟢 No existing tier — first-time selection flow");
      }

      // -------------------------------
      // CUSTOM tier prompt (input)
      // -------------------------------
          ...
          preConfirm: () => {
            ...
            if (unit === "GB" && amount < 50) {
              Swal.showValidationMessage(
                "Minimum capacity for a custom tier is 50 GB"
              );
              return null;
            }
      ...
      // Check for downgrade
      // -------------------------------
      if (currentTierInfo && selectedStorageGB < currentStorageGB) {
        await Swal.fire({
          icon: "warning",
      ...
      // -------------------------------
      // Confirm upgrade (applies to all tiers)
      // -------------------------------
      const confirmUpgrade = await Swal.fire({
        title: "Confirm Upgrade",
        html: `You are currently on <b>${currentTierInfo?.title || "None"}</b> (${currentTierInfo?.storage || "0GB"
          }) and about to upgrade to <b>${selectedTierInfo.title}</b> (${selectedTierInfo.storage
      ...
      // -------------------------------
      // Submit upgrade request (all tiers)
      // -------------------------------
      const payload = {
      ...
      const upgradeRes = await axios.post(`${BASE_URL}/upgrade/request`, payload);

      toast.success(upgradeRes.data.message || "Upgrade successful!");
      await Swal.fire({
        icon: "success",
      ...
      setLoading(false);
      console.log("⏹️ Finished tier selection process");
    }
  };
