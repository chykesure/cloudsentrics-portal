...
const ReportIssue = () => {
  ...
  // 1️⃣ Ensure user confirmed submission
  if (!formData.confirm) {
  ...
  try {
    ...
    const res = await fetch("http://localhost:5000/api/reports", {
      ...
      headers: {
        ...
        "Authorization": `Bearer ${localStorage.getItem("token")}`, // JWT from localStorage
      },
      body: JSON.stringify(payload),
    });

    // 5️⃣ Handle non-200 responses
    if (!res.ok) {
