...
const Issue3 = ({ goBack, formData, setFormData, onSubmit }: StepProps & { onSubmit: () => void }) => {
  ...
  // Wrapper to handle loading state during submission
  const handleSubmitClick = async () => {
    if (!formData.confirm) return;
    setSubmitting(true);
    try {
      await onSubmit(); // call parent submission
    } finally {
      setSubmitting(false);
    }
  };
