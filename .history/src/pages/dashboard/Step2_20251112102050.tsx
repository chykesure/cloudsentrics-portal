...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (!currentTierInfo) {
      ...
      // If selecting CUSTOM for first-time, just set default storage
      let tierDetails;
      if (tierId === "custom") {
        tierDetails = {
          title: "CUSTOM",
    ...
    // -------------------------------
    // CUSTOM TIER FLOW (Existing User)
    // -------------------------------
    if (tierId === "custom") {
      // fetch backend data for current custom storage
      ...
      // confirm upgrade only for existing users
