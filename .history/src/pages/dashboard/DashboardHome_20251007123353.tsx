    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const requestsData: RequestType[] = data.requests || [];
                ...
                setIssues({
                    totalReported: issuesData.totalReported || 0,
                    solved: issuesData.solved || 0,
                    pending: issuesData.pending || 0,
                });
                ...
                setChartData([
                    ...
                    { name: "In progress", value: requestsData.filter(r => r.status === "In Progress").length, color: "#032352" },
                    { name: "Pending", value: statsData.pending || 0, color: "#f97316" },
                ]);

                animateCounts(statsData, issuesData);
            } catch (err) {
                console.error("Error fetching dashboard data:", err);
                // Fallback to empty defaults
                setStats({ totalRequests: 0, approved: 0, pending: 0 });
                setIssues({ totalReported: 0, solved: 0, pending: 0 });
                setRequests([]);
                setChartData([
                    { name: "Approved", value: 0, color: "#16a34a" },
                    { name: "In progress", value: 0, color: "#032352" },
                                    ...
                                    requests.map((row, idx) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-3 px-3">
                                                <span
                                                    className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium shadow-sm ${row.status === "Approved" ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"
                                                        }`}
                                                >
