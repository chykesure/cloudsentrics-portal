...
const ProfilePage = () => {
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://localhost:5002/api/profile/${form.companyEmail}`,
        formData,
        {
          headers: {
  ...
  const renderTierBadge = (tier: string) => {
    // 🛡️ Guard: hide badge for invalid, empty, null, or junk values
    if (
      !tier ||
      tier.trim() === "" ||
    ...
    // ❌ Remove the fallback to "STANDARD TIER"
    const tierStyle = styleMap[normalizedTier];
    if (!tierStyle) return null; // no match → hide badge completely
