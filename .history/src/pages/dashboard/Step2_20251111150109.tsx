...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const fetchBackendTier = async () => {
    ...
    try {
      console.log("✅ Fetching backend tier for:", userEmail);
      const res = await axios.get(`${BASE_URL}/upgrade/request/by-email/${userEmail}`);
      const backendTierData = res.data.data || {};

      console.log("📦 Backend tier data:", backendTierData);

      // Save to localStorage for handleSelectTier
      localStorage.setItem("backendTierData", JSON.stringify(backendTierData));

      // Optionally save in state if you want reactive updates
      setBackendTierData?.(backendTierData);

      // Extract current tier and status
      const backendTier = backendTierData.selectedTier || backendTierData.currentTier || null;
      setCurrentTier(backendTier);
      if (backendTier) setSelectedTier(backendTier);

      // Save upgrade status in formData
      const status = res.data.status || "None";
      setFormData((prev) => ({ ...prev, upgradeStatus: status }));
      ...
      console.error("❌ Failed to fetch backend tier:", err);
      setCurrentTier(null);
    } finally {
      setIsTierLoading(false);
    }
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // If the current tier is CUSTOM, fetch actual storage from user object
    if (currentTier === "CUSTOM") {
      // Try to use backend data first (from last fetch)
      ...
      currentTierInfo = {
        ...
        title: "CUSTOM",
        storage:
          backendData.tierStorage ||       // ✅ Prefer backend "450 GB"
          user.tierStorage ||              // fallback if stored in user object
          "Custom Storage",                // last resort
        rank: 999,
        channels: [],
        response: "",
        availability: "",
    ...
    if (tierId === "custom") {
      ...
      const parseStorageToGB = (storage: string): number => {
        ...
        return unit === "TB" ? value * 1024 : value; // convert TB → GB
      };
      ...
      if (customStorageGB < currentStorageGB) {
        await Swal.fire({
          ...
          text: `Your current storage is ${currentStorageGB} GB. Custom tier cannot be lower than your current storage.`,
          confirmButtonColor: "#032352",
        });
        return;
      }
      ...
      // --- Confirm upgrade
      // ✅ Fix: Determine current storage dynamically
      // --- Confirm upgrade
      let displayCurrentStorage = currentTierInfo?.storage;

      // If it's CUSTOM, prefer the backend value
      if (currentTier === "CUSTOM") {
        // Get backend tier info from localStorage (or from state if you store it there)
        const backendData = JSON.parse(localStorage.getItem("backendTierData") || "{}");
        const user = JSON.parse(localStorage.getItem("user") || "{}");

        displayCurrentStorage =
          backendData.tierStorage ||       // ✅ Prefer backend storage
          currentTierInfo?.storage ||      // fallback to existing info
          user.tierStorage ||              // fallback to user object
          "Custom Storage";                // last resort
      }
      ...
      const confirmUpgrade = await Swal.fire({
        ...
        html: `You are currently on <b>${currentTierInfo?.title || "CUSTOM"}</b> (${displayCurrentStorage}) and about to upgrade to <b>CUSTOM</b> (${customInput.amount} ${customInput.unit}).<br><br>Do you want to continue?`,
        icon: "question",
      ...
      try {
        ...
        await Swal.fire({
          ...
          if (result.isConfirmed) window.location.reload(); // ✅ Reload after OK
        });
      ...
      return; // exit after custom tier flow
    }
