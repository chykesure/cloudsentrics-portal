...
const ProfilePage = () => {
  ...
  const [avatar, setAvatar] = useState(defaultAvatar); // state to hold profile image
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onloadend = () => {
        setAvatar(reader.result as string); // preview uploaded image
      };
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50">
      {/* Cover Image */}
      <div
        className="relative h-66 bg-cover bg-center"
      ...
      <div className="relative flex items-end -mt-16 px-10">
        ...
        <div className="relative">
          <img
            src={avatar}
            alt="Profile"
            className="h-32 w-32 rounded-full border-[6px] border-yellow-400 shadow-lg object-cover"
          />
          {/* Hidden file input */}
          <input
            type="file"
          ...
          <label
            ...
            className="absolute bottom-1 right-1 bg-blue-600 text-white p-1 rounded-full shadow hover:bg-blue-700 cursor-pointer"
          >
