...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (tierId === "custom") {
      ...
      // --- Get actual current tier title and storage safely
      const currentTierTitle =
        currentTierInfo?.title || form.tier || "CUSTOM";
      const currentTierStorage =
        currentTierInfo?.storage || form.storage || "0GB";

      // --- Convert storage to GB for validation
      const currentStorageGB = parseStorageToGB(currentTierStorage);
