...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (currentTierInfo?.title?.toUpperCase() === "CUSTOM") {
      try {
        // Read backend tier data from localStorage
        const backendData = JSON.parse(
          localStorage.getItem("backendTierData") || "{}"
        );

        // Pick storage safely
        const storage = backendData.tierStorage || "50 GB"; // fallback

        // Replace currentTierInfo completely
        currentTierInfo = {
          id: "custom",
          title: "CUSTOM",
          storage, // ✅ this is what the modal will now show
          rank: 999,
          channels: [],
          response: "",
          availability: "",
        ...
        console.log("✅ CUSTOM tier loaded from localStorage:", currentTierInfo);
      } catch (err) {
        console.warn(
          "❌ Could not parse backendTierData from localStorage:",
          err
        );
        currentTierInfo = { ...currentTierInfo, storage: "50 GB" }; // fallback
      }
    }
