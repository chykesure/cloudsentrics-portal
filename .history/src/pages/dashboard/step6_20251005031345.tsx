...
import SuccessScreen from "./SuccessScreen"; // ✅ import success screen
...
const Step6 = ({ goBack, onSubmit, formData }: StepProps) => {
  ...
  const [showSuccess, setShowSuccess] = useState(false); // ✅ state for success screen
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        descriptionADF, // ✅ include it here
      };
      ...
      // ✅ Show the success screen or trigger onSubmit depending on flow
      if (onSubmit) {
        onSubmit();
      } else {
        setShowSuccess(true);
  ...
  // ✅ Once submitted, show success screen
  if (showSuccess) {
