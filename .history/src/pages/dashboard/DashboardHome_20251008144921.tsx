...
const DashboardHome = () => {
  ...
  // Two-color palette: dark blue variations + white
    ...
    <div className="min-h-screen bg-white p-6 sm:p-10 space-y-10">
      ...
      <div className="relative w-full">
        <motion.div
          ...
          className="bg-[#032352] text-white rounded-2xl shadow-lg p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10"
        >
        ...
        {stats.map((stat, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`bg-[${darkBlues[stat.colorIdx]}] text-white rounded-2xl shadow-md p-6 flex items-center gap-4 cursor-pointer transition`}
            >
        ...
        {actions.map((action, idx) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{ scale: 1.03 }}
              className={`bg-[${darkBlues[action.colorIdx]}] text-white rounded-2xl shadow-md p-6 flex flex-col items-start gap-4 hover:shadow-lg transition cursor-pointer`}
            >
      ...
      <div className="grid grid-cols-1 gap-4">
        {announcements.map((ann) => (
          <motion.div
            ...
            className={`border-l-4 border-white bg-[${darkBlues[ann.colorIdx]}] p-4 rounded-lg cursor-pointer transition`}
          >
      ...
      {tipOpen && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className={`bg-[${darkBlues[1]}] text-white rounded-2xl p-6 shadow-md flex flex-col md:flex-row items-center justify-between gap-4`}
        >
