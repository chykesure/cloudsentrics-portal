...
const DashboardHome = () => {
  ...
  // Distinct dark blue shades for cards
  const cardColors = ["bg-[#032352]", "bg-[#043a5e]", "bg-[#054a6b]"];
    ...
    <div className="min-h-screen bg-white p-6 sm:p-10 space-y-10">
      ...
      <div className="relative w-full">
        <motion.div
          ...
          className="bg-gradient-to-r from-[#032352] to-[#043a5e] text-white rounded-2xl shadow-lg p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10"
        >
        ...
        {stats.map((stat, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`${cardColors[stat.colorIdx]} text-white rounded-2xl shadow-lg p-6 flex items-center gap-4 cursor-pointer transition`}
            >
        ...
        <div className="flex flex-col gap-4">
          {announcements.map((ann, idx) => (
            <motion.div
              ...
              className={`${cardColors[idx % cardColors.length]} text-white p-4 rounded-lg cursor-pointer transition`}
            >
              <div className="flex justify-between items-center">
                <h3 className="font-semibold">{ann.title}</h3>
                <span className="text-white/80 text-sm">{new Date(ann.date).toLocaleDateString()}</span>
              </div>
              <p className="mt-1 text-white/80">{ann.message}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      {tipOpen && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className={`${cardColors[1]} text-white rounded-2xl p-6 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4`}
        >
          ...
          <button
            ...
            className="px-6 py-2 bg-white text-[#043a5e] rounded-lg font-semibold hover:bg-gray-100 transition"
          >
            Dismiss
          </button>
        </motion.div>
