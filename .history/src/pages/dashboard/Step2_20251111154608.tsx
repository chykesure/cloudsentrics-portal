...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  // --- Resolve current tier info
  const currentTierIdResolved = resolveTierId(currentTier);
  let currentTierInfo = tiers.find((t) => t.id === currentTierIdResolved) || null;

  const user = JSON.parse(localStorage.getItem("user") || "{}");
  const backendData = JSON.parse(localStorage.getItem("backendTierData") || "{}");

  // --- If current tier is CUSTOM, fetch actual storage
  if (currentTierIdResolved === "custom") {
    const res = await axios.get(`${BASE_URL}/upgrade/request/by-email/${user.email || user.companyEmail}`);
    const data = res.data.data || {};
    const storage = data.tierStorage || user.tierStorage || "Custom Storage";
    currentTierInfo = {
      id: "custom",
      title: "CUSTOM",
      storage,
  ...
  // --- Selected tier info
  const selectedTierInfo = tiers.find((t) => t.id === tierId) || null;
  if (!selectedTierInfo && tierId !== "custom") {
    console.warn("❌ No tier found for ID:", tierId);
    return;
  }

  // --- Handle CUSTOM tier input
  let selectedStorageGB: number;
  let displaySelectedStorage: string;
  ...
  if (tierId === "custom") {
    ...
    selectedStorageGB = parseStorageToGB(`${customInput.amount} ${customInput.unit}`);
    displaySelectedStorage = `${customInput.amount} ${customInput.unit}`;

    const currentStorageGB = parseStorageToGB(currentTierInfo?.storage || "0GB");
    ...
    // Confirm custom upgrade
    const confirmUpgrade = await Swal.fire({
      title: "Confirm Upgrade",
      html: `You are currently on <b>${currentTierInfo?.title || "CUSTOM"}</b> (${currentTierInfo?.storage}) and about to upgrade to <b>CUSTOM</b> (${displaySelectedStorage}).<br><br>Do you want to continue?`,
      icon: "question",
    ...
    try {
      ...
      const payload = {
        ...
        newStorage: displaySelectedStorage,
      ...
      await Swal.fire({
        ...
        text: `Your custom tier upgrade to ${displaySelectedStorage} has been submitted successfully.`,
        confirmButtonColor: "#032352",
      ...
      setFormData((prev) => ({
        ...prev,
        selectedTier: "custom",
        tierDetails: { title: "CUSTOM", storage: displaySelectedStorage, extras: customInput.note },
      }));
      setIsNextEnabled(true);
    } catch (err: any) {
      console.error("❌ Custom upgrade request failed:", err);
      Swal.fire({ icon: "error", title: "Upgrade Request Failed", text: err.response?.data?.message || "An error occurred while submitting your custom upgrade request.", confirmButtonColor: "#032352" });
    } finally {
      setLoading(false);
  ...
  // --- Normal Tier Flow
  selectedStorageGB = parseStorageToGB(selectedTierInfo!.storage);
  const currentStorageGB = parseStorageToGB(currentTierInfo?.storage || "0GB");
  ...
  try {
    ...
    const res = await axios.post(`${BASE_URL}/upgrade/request`, payload);
    toast.success(res.data.message || "Upgrade successful!");
    await Swal.fire({ icon: "success", title: "Upgrade Request Submitted", text: `Your upgrade to ${selectedTierInfo!.title} has been submitted successfully.`, confirmButtonColor: "#032352" }).then((r) => r.isConfirmed && window.location.reload());
    ...
    Swal.fire({ icon: "error", title: "Upgrade Request Failed", text: err.response?.data?.message || "An error occurred while submitting your upgrade request.", confirmButtonColor: "#032352" });
  } finally {
    setLoading(false);
