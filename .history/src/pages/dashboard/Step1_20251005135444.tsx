          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 mt-10">
            ...
            <button
              ...
              disabled={
                ...
                !selectedStorageCount ||
                !(
                  Object.values(storageNames).some((name) => name.trim() !== "") ||
                  bucketNote.trim() !== ""
                )
              }
      ...
      <button
        onClick={() => {
          if (selectedOption === "change") {
            ...
            formData.append("existingAccountId", existingAccountId);
            formData.append("existingStorageName", existingStorageName);
            formData.append("details", details);
            changesRequested.forEach((option, index) => {
              formData.append(`changesRequested[${index}]`, option);
            });

            // SEND FORM DATA TO STEP6
            jumpToStep?.(6, formData);
          }
        }}
        ...
        className={`w-full sm:w-auto px-8 py-3 sm:px-10 sm:py-4 text-base sm:text-lg rounded-md ${
          !selectedOption || selectedOption !== "change"
            ? "bg-gray-300 text-gray-500 cursor-not-allowed"
            : "bg-[#032352] text-white hover:bg-blue-700"
        }`}
      >
