...
const DashboardHome = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                const { companyEmail } = JSON.parse(storedUser);

                const res = await axios.get(`/api/dashboard/${companyEmail}`);
    ...
    // Pie chart data dynamically
    const pieData = [
        { name: "Approved", value: stats.approved, color: "#16a34a" },
        { name: "In progress", value: stats.totalRequests - stats.approved - stats.pending, color: "#032352" },
        { name: "Pending", value: stats.pending, color: "#f97316" },
    ];

    const statsArray = [
        { label: "Total Portal Request", value: stats.totalRequests, icon: FileText },
        { label: "Approved Portal Request", value: stats.approved, icon: CheckCircle },
        { label: "Pending Portal Request", value: stats.pending, icon: Clock },
    ];

    const issuesArray = [
        { label: "Total Issue Reported", value: issues.totalReported, icon: AlertTriangle, color: "bg-[#032352]" },
        { label: "Total Issue Solved", value: issues.solved, icon: CheckSquare, color: "bg-green-600" },
        { label: "Pending Issues", value: issues.pending, icon: XCircle, color: "bg-[#032352]" },
    ];
            ...
            <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {statsArray.map((item, idx) => {
                        ...
                        const gradients = ["from-[#032352] to-[#3b82f6]", "from-emerald-600 to-teal-500", "from-rose-500 to-orange-400"];
                    ...
                    <div className="overflow-x-auto rounded-lg border border-gray-200">
                        <table className="min-w-full table-fixed border-collapse text-sm">
                            ...
                            <tbody>
                                {requests.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="py-6 text-center text-gray-500 italic">No requests found</td>
                                    </tr>
                                ) : (
