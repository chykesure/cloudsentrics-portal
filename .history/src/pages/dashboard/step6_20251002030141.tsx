...
const Step6 = ({ goBack, onSubmit, formData, user }: StepProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1️⃣ Build AWS accounts from Step1
      ...
      // 2️⃣ Build Jira description in proper Atlassian Document Format
        ...
        content: [
          {
            ...
            content: [
              {
                type: "text",
                text: `Reporter: ${user?.companyEmail || "N/A"}`
              }
            ]
          },
          {
            ...
            content: [
              {
                type: "text",
                text: "AWS Accounts:\n" + awsAccounts.map(a => `${a.alias} — ${a.orgName}`).join("\n")
              }
            ]
          },
          {
      ...
      // 3️⃣ Build payload to send to backend
      const payload = {
        title: "Request",
        reporterEmail: user?.companyEmail || "N/A",
        awsAccounts,
        acknowledgements,
        descriptionADF, // send this to backend to map to Jira description
      };
