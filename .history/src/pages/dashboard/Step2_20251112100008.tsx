...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  // -------------------------------
  // Unified email reference
  // -------------------------------
  const user = JSON.parse(localStorage.getItem("user") || "{}");
  const email = user.companyEmail || user.email;

  // -------------------------------
  // Helper: Normalize tier ID
  // -------------------------------
  ...
  // -------------------------------
  // Helper: Parse storage string to GB
  // -------------------------------
  ...
  if (currentTierIdResolved === "custom") {
    try {
      const res = await axios.get(`${BASE_URL}/upgrade/request/by-email/${email}`);
      ...
      localStorage.setItem("backendTierData", JSON.stringify({ tierStorage: storage }));
    } catch (err) {
      console.warn("❌ Could not fetch backend custom tier data:", err);
      const storage = user.tierStorage || "Custom Storage";
      currentTierInfo = {
        id: "custom",
        title: "CUSTOM",
        storage,
  ...
  if (tierId === "custom") {
    ...
    // -------------------------------
    // Fetch existing custom request
    // -------------------------------
    let existingCustom: any = null;
    try {
      const res = await axios.get(`${BASE_URL}/request/by-email/${email}`);
      existingCustom = res.data?.data || null;
      if (existingCustom && existingCustom.selectedTier !== "custom") existingCustom = null;
    } catch (err: any) {
      if (err.response?.status !== 404) {
        console.error("❌ Could not fetch previous custom request", err);
        Swal.fire({
          icon: "error",
          title: "Error Fetching Request",
          text: "Could not check existing custom tier request. Please try again.",
          confirmButtonColor: "#032352",
        });
        return;
      }
    ...
    if (existingCustom && newStorageGB < currentStorageGB) {
      await Swal.fire({
        icon: "warning",
    ...
    try {
      setLoading(true);
      if (!existingCustom) {
        console.log("🚀 Creating new custom request:", payload);
        await axios.post(`${BASE_URL}/upgrade/request`, payload);
      } else {
        console.log("🔄 Updating existing custom request:", payload);
        await axios.put(`${BASE_URL}/upgrade/request/${existingCustom._id}`, payload);
      }

      toast.success("Custom tier saved successfully!");
      await Swal.fire({
        icon: "success",
        title: "Tier Saved",
        text: `Your custom tier of ${customInput.amount} ${customInput.unit} has been recorded.`,
        confirmButtonColor: "#032352",
      });
      ...
      Swal.fire({
        ...
        text: "An error occurred while submitting your custom tier request.",
        confirmButtonColor: "#032352",
      });
    } finally {
      setLoading(false);
  ...
  // -------------------------------
  // STANDARD / BUSINESS / PREMIUM Flow
  // -------------------------------
  const selectedTierInfo = tiers.find((t) => t.id === tierId);
  if (!selectedTierInfo) return console.warn("❌ No tier found for ID:", tierId);
  ...
  try {
    ...
    console.log("🚀 Sending upgrade payload:", payload);
    await axios.post(`${BASE_URL}/upgrade/request`, payload);

    toast.success("Upgrade submitted successfully!");
    ...
    Swal.fire({
      ...
      text: "An error occurred while submitting your upgrade request.",
      confirmButtonColor: "#032352",
    });
  } finally {
    setLoading(false);
