    ...
    const fetchProfile = async () => {
      try {
        ...
        setForm({
          ...
          phone: data.phone || "",
          tier: data.tier || "STANDARD TIER",
          storage: data.storage || "200GB",
        });
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (updatedProfile.avatar) {
          const fullAvatarUrl = `http://localhost:5000${updatedProfile.avatar}`;
  ...
  return (
    <div className="min-h-[80vh] w-full bg-gradient-to-b from-gray-100 to-gray-50 pb-16 relative">
      ...
      >
        ...
        >
          <h2 className="text-2xl sm:text-4xl font-extrabold tracking-wide drop-shadow-md">
            Cloud Sentrics
          </h2>
          <p className="text-sm sm:text-lg opacity-90 mt-2 font-medium">
            Profile Settings
          </p>
        </motion.div>
      </div>
      ...
      <div className="max-w-7xl mx-auto -mt-14 sm:-mt-20 px-4 sm:px-8 mb-16">
        <div className="flex flex-col md:flex-row justify-center items-start gap-6 md:gap-10">
          ...
          <motion.div
            ...
            className="flex-1 w-full max-w-md bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl p-6 sm:p-10 border border-gray-200 text-center"
          >
            <div className="relative inline-block mb-5 group">
              <label htmlFor="avatar-upload" className="cursor-pointer block">
                <img
                  ...
                  alt="Avatar"
                  className="w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-[#032352]/20 object-cover shadow-xl transition-transform group-hover:scale-105"
                />
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-[#032352]">
              {form.name || "User"} 👋
            </h3>
            <p className="text-gray-600 mt-1 text-sm sm:text-base">{form.companyEmail}</p>
          ...
          <motion.div
            ...
            className="flex-1 w-full max-w-md bg-white/80 backdrop-blur-lg rounded-2xl shadow-lg p-6 sm:p-10 border border-gray-200"
          >
            <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
              <h3 className="text-lg sm:text-xl font-semibold text-[#032352]">
                Update Profile
              </h3>
              <button
                onClick={() => setIsEditing(true)}
                className="px-4 sm:px-5 py-2 bg-[#032352] text-white rounded-lg shadow hover:bg-[#021a3d] transition font-medium text-sm sm:text-base"
              >
                Edit
              </button>
            </div>
      ...
      >
        ...
        <h3 className="text-sm sm:text-base font-bold tracking-wide uppercase drop-shadow-lg">
          © {new Date().getFullYear()} Cloud Sentrics
        </h3>
        <p className="text-xs sm:text-sm mt-2 text-gray-200 opacity-90 font-medium">
