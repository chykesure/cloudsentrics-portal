...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    try {
      ...
      // -------------------------------
      // Compare with current tier (upgrade / first-time / invalid)
      // -------------------------------
      ...
      if (!currentTierInfo) {
        ...
        console.log("🟢 First-time tier selection detected");
      ...
      const payload = {
        ...
        note: selectedTierInfo.note || "",
        status: "pending",
        timestamp: new Date().toISOString(),
      };

      console.log("🚀 Sending unified upgrade payload:", payload);
      ...
      setFormData((prev) => ({
        ...
        tierDetails: {
          title: selectedTierInfo.title,
          storage: selectedTierInfo.storage,
          extras: selectedTierInfo.note || selectedTierInfo.extras,
        },
      }));
      setIsNextEnabled(true);
    } catch (err: any) {
