...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfileAndRequest = async () => {
      try {
        // Fetch profile
        const profileRes = await axios.get<ProfileData>(`http://localhost:5000/api/profile/${email}`);
        const profileData = profileRes.data;

        // Fetch latest request for tier & storage
        const requestsRes = await axios.get<any[]>(
          `http://localhost:5000/api/requests?reporterEmail=${email}`
        );
        const latestRequest = requestsRes.data?.[0];

        if (latestRequest) {
          profileData.tier = latestRequest.selectedTier;
          profileData.storage = latestRequest.selectedStorageCount.toString();
        }
