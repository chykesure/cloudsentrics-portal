...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (tierId === "custom") {
      ...
      // ✅ Check if previous CUSTOM request exists
      let previousCustomRequest = null;
      try {
        const res = await axios.get(
          `${BASE_URL}/request/by-email/${user.email || user.companyEmail}?tier=custom`
        );
        previousCustomRequest = res.data?.data || null;
      } catch (err) {
        console.warn("❌ Could not fetch previous custom request", err);
      }
      ...
      // 🛑 Prevent downgrade
      if (previousCustomRequest && newStorageGB < currentStorageGB) {
        await Swal.fire({
          icon: "warning",
      ...
      // 🧠 Create or Update depending on existence (simplified + cleaner)
try {
  setLoading(true);

  const userEmail = user.companyEmail || user.email;

  // ✅ Try to find any existing custom tier request by email
  const { data: existing } = await axios.get(
    `${BASE_URL}/request/by-email/${userEmail}`
  );

  if (existing && existing.selectedTier === "custom") {
    console.log("🔄 Updating existing custom request:", payload);
    await axios.put(`${BASE_URL}/request/${existing._id}`, payload);
  } else {
