...
const Step6 = ({ goBack, formData }: StepProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ Build STORAGE accounts (if any)
      const storageAccounts: { alias: string; orgName: string }[] = [];
      Object.entries(formData.storageAliases || {}).forEach(([label, orgName]) => {
        if (orgName.trim() !== "") {
          storageAccounts.push({ alias: `cloudsentrics-storage-${label}`, orgName });
        }
      });
      ...
      if (isAWS && awsAccounts.length > 0) {
        ...
        descriptionContent.push({
          type: "paragraph",
          content: [{ type: "text", text: `Number of AWS Account(s) Needed: ${awsCount}` }],
        });
      ...
      if (!isAWS && storageAccounts.length > 0) {
        ...
        descriptionContent.push({
          type: "paragraph",
          content: [{ type: "text", text: `Number of STORAGE Account(s) Needed: ${storageCount}` }],
        });
