...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (!currentTierInfo) {
      ...
      if (tierId === "custom") {
        // CUSTOM first-time: show input for storage
        ...
        // ✅ Update formData with custom amount/unit
        setFormData((prev) => ({
          ...prev,
      ...
      await Swal.fire({
        icon: "success",
        title: "Tier Selected",
        text: `You’ve selected the ${tierDetails.title}.`,
        confirmButtonColor: "#032352",
      });
    ...
    // -------------------------------
    // EXISTING USER: CUSTOM TIER FLOW
    // -------------------------------
    if (tierId === "custom") {
      let storage = currentTierInfo.storage;
      try {
        const res = await axios.get(
          `${BASE_URL}/upgrade/request/by-email/${user.email || user.companyEmail}`
        );
        const backendData = res.data.data || {};
        storage = backendData.tierStorage || currentTierInfo?.storage || "0 GB";
      ...
      const confirmUpgrade = await Swal.fire({
        ...
        html: `You are currently on <b>${currentTierInfo.title}</b> (${currentTierInfo.storage}) and about to upgrade to <b>CUSTOM</b> (${customInput.amount} ${customInput.unit}).<br><br>Do you want to continue?`,
        icon: "question",
    ...
    // -------------------------------
    // NORMAL TIER FLOW (Existing User)
    // -------------------------------
    const selectedTierInfo = tiers.find((t) => t.id === tierId);
    // If the current tier is CUSTOM, make sure we have storage from backend
    if (currentTierInfo?.title?.toUpperCase() === "CUSTOM") {
      try {
        const backendTierRes = await axios.get(
          `${BASE_URL}/upgrade/request/by-email/${user.email || user.companyEmail}`
        );
        const backendData = backendTierRes.data.data || {};
        currentTierInfo.storage = backendData.tierStorage || "50 GB"; // fallback
        localStorage.setItem("backendTierData", JSON.stringify({ tierStorage: currentTierInfo.storage }));
      } catch (err) {
        console.warn("❌ Could not fetch backend custom tier data:", err);
        // fallback if backend fails
        currentTierInfo.storage = "50 GB";
      }
    }
    ...
    // Ensure storage is populated for CUSTOM tier
    if (currentTierInfo.title?.toUpperCase() === "CUSTOM") {
      const backendTier = JSON.parse(localStorage.getItem("backendTierData") || "{}");
      // Force a default if empty
      if (!currentTierInfo.storage || currentTierInfo.storage.trim() === "") {
        currentTierInfo.storage = backendTier?.tierStorage || "50 GB";
      }
    }
    ...
    const confirmUpgrade = await Swal.fire({
      title: "Confirm Upgrade",
      html: `You are currently on <b>${currentTierInfo.title}</b> (${currentTierInfo.storage}) 
and about to upgrade to <b>${selectedTierInfo.title}</b> (${selectedTierInfo.storage}).<br><br>
