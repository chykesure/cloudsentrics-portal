...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // -------------------------------
    // Fetch backend info if custom tier
    // -------------------------------
    if (currentTierIdResolved === "custom") {
      try {
        ...
        const storage = backendData.tierStorage || user.tierStorage || null;
        ...
        currentTierInfo = null; // treat as first-time if fetch fails
      }
    }
    ...
    // -------------------------------
    if (tierId === "custom") {
      const isFirstTime = !currentTierInfo || !currentTierInfo.storage;

      // Prompt user for custom storage input
      const { value: customInput } = await Swal.fire({
        title: isFirstTime ? "Custom Tier Setup" : "Custom Tier Upgrade",
        html:
      ...
      if (!isFirstTime) {
        // Upgrade flow: prevent downgrade
      ...
      try {
        ...
        const payload = {
          ...
          previousTier: isFirstTime ? undefined : currentTierInfo?.title,
          previousStorage: isFirstTime ? undefined : currentTierInfo?.storage,
          newTier: "CUSTOM",
          newStorage: `${customInput.amount} ${customInput.unit}`,
          note: customInput.note || "",
          status: isFirstTime ? "active" : "pending",
        ...
        toast.success(
          res.data.message ||
          (isFirstTime ? "Storage setup successful!" : "Upgrade successful!")
        );

        await Swal.fire({
          icon: "success",
          title: isFirstTime ? "Storage Setup Completed" : "Upgrade Request Submitted",
          text: `Your custom tier ${isFirstTime ? "setup" : "upgrade"} to ${customInput.amount
            } ${customInput.unit} has been submitted successfully.`,
          confirmButtonColor: "#032352",
        });
        ...
        if (!isFirstTime) window.location.reload(); // reload only for upgrades
      } catch (err: any) {
        console.error("❌ Custom request failed:", err);
        Swal.fire({
          icon: "error",
          title: "Request Failed",
          text: err.response?.data?.message || "An error occurred while submitting your request.",
          confirmButtonColor: "#032352",
        });
      } finally {
        setLoading(false);
    ...
    // FIRST-TIME selection (no existing storage)
    if (!currentTierInfo || !currentTierInfo.storage) {
