...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // If CUSTOM, fetch backend storage
    if (currentTierIdResolved === "custom") {
      try {
        ...
        // Save backend tier data for later use
...
// -------------------------------
if (tierId === "custom") {
  // Ensure we get a valid userId from localStorage
  const userId =
    user._id || user.id || user.userId || user.uid || null;

  if (!userId) {
    Swal.fire({
      icon: "error",
      title: "User ID Missing",
      text: "Cannot submit request: userId is missing. Please log in again.",
      confirmButtonColor: "#032352",
    });
    return;
  }
  ...
  if (!isFirstTime) {
    ...
    // Confirm upgrade for existing users
  ...
  try {
    ...
    const payload = {
      ...
      previousTier: isFirstTime ? null : currentTierInfo?.title,
      newTier: "CUSTOM",
      previousStorage: isFirstTime ? null : currentTierInfo?.storage,
    ...
    toast.success(
      res.data.message || (isFirstTime ? "Storage setup successful!" : "Upgrade successful!")
    );

    await Swal.fire({
      icon: "success",
      title: isFirstTime ? "Storage Setup Completed" : "Upgrade Request Submitted",
      text: `Your custom tier ${isFirstTime ? "setup" : "upgrade"} to ${customInput.amount} ${customInput.unit} has been submitted successfully.`,
      confirmButtonColor: "#032352",
    });
    ...
    if (!isFirstTime) {
      window.location.reload(); // reload only after upgrade
    }
  } catch (err: any) {
