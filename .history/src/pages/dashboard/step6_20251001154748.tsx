...
interface StepProps {
  goBack: () => void;
  goNext?: () => void; // optional, if needed
  jumpToStep?: (step: number) => void; // optional
  formData: any;
  setFormData: React.Dispatch<React.SetStateAction<any>>;
}

const Step6 = ({ goBack, goNext, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Save into formData before request
      ...
      // Move wizard forward if goNext is provided
      if (goNext) goNext();
    } catch (error) {
      console.error(error);
