...
const Step1 = ({ goNext, goBack }: StepProps) => {
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap gap-6 text-xl">
          ...
          <label className="flex items-center space-x-2">
            <input
              ...
              checked={selectedOptions.includes("aws")}
              onChange={() => toggleOption("aws")}
            />
          ...
          <label className="flex items-center space-x-2">
            <input
              type="checkbox"
              className="form-checkbox w-6 h-6"
              checked={selectedOptions.includes("storage")}
          ...
          <label className="flex items-center space-x-2">
            <input
              type="checkbox"
              className="form-checkbox w-6 h-6"
              checked={selectedOptions.includes("change")}
              onChange={() => toggleOption("change")}
            />
      ...
      {/* 2. Storage(s) */}
      {selectedOptions.includes("storage") && (
        <div className="mb-12 p-6 border rounded-md bg-gray-50">
          <p className="text-gray-600">
            📦 Storage(s) section placeholder — we’ll add fields here later.
          </p>
        </div>
      )}

      {/* 3. Change to Existing */}
      {selectedOptions.includes("change") && (
        <div className="mb-12 p-6 border rounded-md bg-gray-50">
          <p className="text-gray-600">
            ⚙️ Change to Existing Account/Storage section placeholder — waiting for your details.
          </p>
        </div>
      )}
      ...
      <div className="flex justify-between mt-10">
        ...
        <button
          onClick={goNext}
          className="px-10 py-4 bg-[#032352] text-white text-lg rounded-md hover:bg-blue-700"
        >
          Next →
        </button>
      </div>
