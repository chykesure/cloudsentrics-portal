...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (tierId === "custom") {
      ...
      // 🟣 CUSTOM first-time selection — same as other tiers
      if (!currentTierInfo) {
        const payload = {
        ...
        setIsNextEnabled(true);
        return; // ❌ skip upgrade path
      }

      // Existing user — handle upgrade only
      ...
      try {
        ...
        toast.success("Custom upgrade request submitted!");
        await Swal.fire({
          icon: "success",
