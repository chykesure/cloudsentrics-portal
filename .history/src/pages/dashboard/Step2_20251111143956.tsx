...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const backendTier = response.data.tierTitle;
const backendTierStorage = response.data.tierStorage;
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    if (currentTier === "CUSTOM") {
      ...
      currentTierInfo = {
        id: "custom",
        title: "CUSTOM",
        storage: user.tierStorage || "0GB", // get actual current storage
        rank: 999, // treat CUSTOM as highest rank
        channels: [],
        response: "",
        availability: "",
        extras: "",
    ...
    if (tierId === "custom") {
      ...
      const confirmUpgrade = await Swal.fire({
        ...
        html: `You are currently on <b>${backendTier} (${backendTierStorage || "N/A"}) </b> (${displayCurrentStorage}) and about to upgrade to <b>CUSTOM</b> (${customInput.amount} ${customInput.unit}).<br><br>Do you want to continue?`,
        icon: "question",
