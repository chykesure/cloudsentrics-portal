...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // ✅ Handle case: user currently on Custom tier with no storage info
    if (
      currentTierInfo.title?.toUpperCase() === "CUSTOM" &&
      (!currentTierInfo.storage || currentTierInfo.storage.trim() === "")
    ) {
      const backendTier = JSON.parse(localStorage.getItem("backendTierData") || "{}");
      currentTierInfo.storage = backendTier.tierStorage || "50 GB"; // hardcoded fallback
    }
    ...
    // ✅ Ensure storage info is not empty for Custom tier
if (
  currentTierInfo?.title?.toUpperCase() === "CUSTOM" &&
  (!currentTierInfo?.storage || currentTierInfo?.storage.trim() === "")
) {
  ...
  currentTierInfo.storage = backendTier?.tierStorage || "50 GB";
    ...
    const confirmUpgrade = await Swal.fire({
      title: "Confirm Upgrade",
      html: `
    You are currently on <b>${currentTierInfo.title}</b> (${currentTierInfo.storage}) 
