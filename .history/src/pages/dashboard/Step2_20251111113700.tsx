...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  // --- Custom Tier Input (for both first-time custom or upgrade from custom)
  if (tierId === "custom") {
    ...
    // --- Convert storage to GB for downgrade check
  ...
  // --- Upgrade Confirmation Modal (skip only if first-time selection)
  if (currentTierInfo) {
    const confirmUpgrade = await Swal.fire({
      title: "Confirm Upgrade",
      html: `You are currently on <b>${currentTierInfo.title}</b> (${currentTierInfo.storage}) and about to upgrade to <b>${tierTitle}</b> (${tierStorage}).<br><br>Do you want to continue?`,
      icon: "question",
  ...
  try {
    ...
    // --- Unified success modal with page reload
    Swal.fire({
      icon: "success",
