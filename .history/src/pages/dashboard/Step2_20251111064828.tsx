...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  console.log("Current Tier (raw):", currentTier);
  console.log("Current Tier resolved:", currentTierIdResolved);
  console.log("Current Tier info:", currentTierInfo);

  let finalTierInfo: any = null;
  ...
  if (tierId === "custom") {
    ...
    finalTierInfo = {
      ...
      rank: currentTierInfo?.rank || 0, // treat as same rank to allow saving
    };
  } else {
    // --- Normal tier
    finalTierInfo = tiers.find((t) => t.id === tierId);
    if (!finalTierInfo) {
      console.warn("❌ No tier found for ID:", tierId);
      return;
    }
    if (currentTierInfo && finalTierInfo.rank < currentTierInfo.rank) {
      await Swal.fire({
        icon: "warning",
        title: "Downgrades Not Allowed",
  ...
  // --- Confirm upgrade if normal tier (skip confirmation for custom)
  if (tierId !== "custom" && currentTierInfo) {
    const confirmUpgrade = await Swal.fire({
      title: "Confirm Upgrade",
      html: `You are currently on <b>${currentTierInfo.title}</b> (${currentTierInfo.storage}) and about to upgrade to <b>${finalTierInfo.title}</b> (${finalTierInfo.storage}).<br><br>Do you want to continue?`,
      icon: "question",
  ...
  setSelectedTier(finalTierInfo.id);
  setFormData((prev) => ({
    ...prev,
    selectedTier: finalTierInfo.id,
    tierDetails: {
      title: finalTierInfo.title,
      storage: finalTierInfo.storage,
      channels: finalTierInfo.channels || "",
      response: finalTierInfo.response || "",
      availability: finalTierInfo.availability || "",
      extras: finalTierInfo.extras || (customInput?.note || ""),
    },
    customTierRequest: customInput || null,
  }));
  setIsNextEnabled(true);

  // --- Send upgrade request to backend (for both normal & custom)
  try {
    setLoading(true);
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    if (!user.email) {
    ...
    const payload = {
      ...
      previousTier: currentTierInfo?.title || "None",
      newTier: finalTierInfo.title,
      previousStorage: currentTierInfo?.storage || "0GB",
      newStorage: finalTierInfo.storage,
      status: "pending",
      timestamp: new Date().toISOString(),
    };

    const res = await axios.post(`${BASE_URL}/upgrade/request`, payload);
    console.log("✅ Upgrade submitted:", res.data);

    await Swal.fire({
      icon: "success",
      title: "Upgrade Request Submitted",
      text: `Your request for ${finalTierInfo.title} has been submitted successfully.`,
      confirmButtonColor: "#032352",
    }).then((result) => {
    ...
    setLoading(false);
    console.log("⏹️ Finished handleSelectTier");
  }
};
