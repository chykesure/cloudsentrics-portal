  ...
  <div className="w-full px-2 sm:px-6 mb-12">
    ...
    <div className="flex flex-col sm:flex-row sm:items-center mb-8 gap-4 text-base sm:text-xl">
      ...
      <input
        ...
        className="w-full sm:w-64 px-4 py-2 border border-gray-300 rounded-md text-base sm:text-lg"
        value={selectedStorageCount ?? ""}
        onChange={(e) => {
          const val = parseInt(e.target.value);
          const count = isNaN(val) || val < 1 ? null : val;
    ...
    {selectedStorageCount && (
      <>
        <b className="block text-base sm:text-lg mb-2">Preferred AWS Alias for each account</b>
        <p className="text-sm sm:text-base mb-4">
          Note: Provide the Organization Name / Department using the account
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
          {Array.from({ length: selectedStorageCount }).map((_, idx) => {
