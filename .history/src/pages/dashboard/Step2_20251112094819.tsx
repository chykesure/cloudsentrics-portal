...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // -------------------------------
    // Get current user and tier info
    // -------------------------------
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    // If CUSTOM tier, fetch backend data
    if (currentTierIdResolved === "custom") {
      try {
        ...
        localStorage.setItem(
          "backendTierData",
          JSON.stringify({ tierStorage: storage })
        );
      } catch (err) {
    ...
    if (tierId === "custom") {
      ...
      // -------------------------------
      // 🧠 Fetch existing custom request safely
      // -------------------------------
      let existingCustom: any = null;
      try {
        const res = await axios.get(`${BASE_URL}/request/by-email/${user.email || user.companyEmail}`);
        existingCustom = res.data?.data || null;
        // Only consider it if it is custom tier
        if (existingCustom && existingCustom.selectedTier !== "custom") {
          existingCustom = null;
        }
      } catch (err: any) {
        if (err.response?.status === 404) {
          existingCustom = null; // first-time request
        } else {
      ...
      // 🧠 Compute GB values for comparison
      ...
      // 🧩 Unified payload
      const payload = {
        reporterEmail: user.companyEmail || user.email,
        title: `Request from ${user.companyEmail || user.email}`,
        selectedTier: "custom",
      ...
      return; // 🔒 stop here — no other flow continues
    }
    ...
    // 🧠 Compare current vs selected storage
    const currentStorageGB = parseStorageToGB(currentTierInfo?.storage);
