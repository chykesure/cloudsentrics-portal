...
import { useState, useEffect,otion } from "framer-motion";
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/onboarding/${form.companyEmail}`,
        {
        ...
        <div className="flex flex-col lg:flex-row justify-center items-start gap-10 lg:gap-14">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {["primaryName", "companyEmail", "primaryPhone"].map((key) => (
