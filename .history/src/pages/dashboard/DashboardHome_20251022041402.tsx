...
import { Settings, FileText, Bell, Star, User } from "lucide-react";
...
const DashboardHome = () => {
  ...
  // Quick Stats
  const stats: Stat[] = [
    { title: "Profile", Icon: User, color: "from-blue-400 to-blue-600", to: "/dashboard/profile" },
    { title: "Request Portal", Icon: FileText, color: "from-green-400 to-green-600", to: "/dashboard/request-portal" },
    { title: "Report Issue", Icon: Bell, color: "from-red-400 to-red-600", to: "/dashboard/report-issue" },
  ];
  ...
  const StatCard = ({ title, Icon, color, to }: Stat) => (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      whileHover={{ scale: 1.05 }}
      className={`bg-gradient-to-r ${color} text-white rounded-2xl shadow-lg p-6 flex items-center gap-4 cursor-pointer transition`}
      onClick={() => navigate(to)}
    >
