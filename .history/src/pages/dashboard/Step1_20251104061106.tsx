      ...
      {selectedOption === "aws" && (
        <div className="w-full px-2 sm:px-6 mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
            {Array.from({ length: Math.min(selectedStorageCount || 0, 6) }).map((_, idx) => {
      ...
      {selectedOption === "storage" && (
        <div className="mb-12">
          ...
          <div className="flex flex-wrap items-center mb-8 gap-4 text-base sm:text-xl">
            <label className="font-medium">Number of Storage(s) (Bucket(s)) required:</label>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setFormData((prev) => ({ ...prev, selectedStorageCount: isNaN(val) ? null : val }));
              }}
              value={selectedStorageCount ?? ""}
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
            {["A", "B", "C", "D", "E", "F"].map((label, idx) => {
              const isDisabled = !selectedStorageCount || selectedStorageCount < idx + 1;
              return (
                <div key={label}>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setStorageNames((prev) => {
                          const updated = { ...prev, [label]: val };
                          setFormData((prevData) => ({ ...prevData, storageNames: updated }));
                          return updated;
                        });
                      }}
                      disabled={isDisabled}
                      className={`flex-1 px-4 py-3 border border-gray-300 rounded-b-md sm:rounded-r-md text-sm sm:text-lg ${isDisabled ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-white text-gray-800"
                        }`}
                    />
                  </div>
                </div>
          ...
          <div className="mb-10">
            ...
            ></textarea>
            <div className="text-right text-sm sm:text-md text-gray-500 mt-1">{bucketNote.length}/{maxLength}</div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 mt-10">
            ...
            <button
              ...
              className={`w-full sm:w-auto px-8 py-3 sm:px-10 sm:py-4 text-base sm:text-lg rounded-md ${!isStorageFormValid() ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-[#032352] text-white hover:bg-blue-700"
                }`}
            >
              Next →
            </button>
