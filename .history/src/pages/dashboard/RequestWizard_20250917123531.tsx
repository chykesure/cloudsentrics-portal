...
useEffect(() => {
  ...
  const goBack = () => {
    setCurrentStep((prev) => {
      if (prev === 6) return 1; // 👈 If on Step6, jump straight to Step1
      return Math.max(prev - 1, 1); // Otherwise normal back behavior
    });
  };
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 p-10 relative">
      ...
      <div className="flex justify-between items-center mb-10">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="flex-1 flex items-center">
            <div
              className={`w-6 h-6 rounded-full ${i + 1 <= currentStep ? "bg-[#032352]" : "bg-gray-300"
                }`}
            />
            {i < 5 && (
              <div
                className={`flex-1 h-1 mx-2 ${i + 1 < currentStep ? "bg-[#032352]" : "bg-gray-300"
                  }`}
              />
            )}
          </div>
      ...
      {showSuccess ? (
        ...
        <Step1 goNext={goNext} goBack={goBack} jumpToStep={jumpToStep} />
      ) : currentStep === 2 ? (
        <Step2 goNext={goNext} goBack={goBack} jumpToStep={jumpToStep} />
      ) : currentStep === 3 ? (
        <Step3 goNext={goNext} goBack={goBack} jumpToStep={jumpToStep} />
      ) : currentStep === 4 ? (
        <Step4 goNext={goNext} goBack={goBack} jumpToStep={jumpToStep} />
      ) : currentStep === 5 ? (
        <Step5 goNext={goNext} goBack={goBack} jumpToStep={jumpToStep} />
      ) : currentStep === 6 ? (
