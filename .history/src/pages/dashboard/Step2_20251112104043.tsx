...
import { ReportFormData } from "../dashboard/types"; // adjust path as needed
...
const handleSelectTier = async (tierId: string) => {
  ...
  let currentTierInfo =
    currentTierIdResolved ? tiers.find((t) => t.id === currentTierIdResolved) || null : null;
  ...
  if (!currentTierInfo) {
    ...
    let tierDetails: ReportFormData["tierDetails"];
    let payload: any = {
    ...
    if (tierId === "custom") {
      ...
      // Add to payload
      payload.newTier = "CUSTOM";
      payload.newStorage = `${customInput.amount} ${customInput.unit}`;
    } else {
      ...
      payload.newTier = selectedTierInfo.title;
      payload.newStorage = selectedTierInfo.storage;
    }

    // -------------------------------
    // Save state & send payload
    // -------------------------------
    ...
    try {
      ...
      toast.success(res.data.message || "Tier selected successfully!");
    } catch (err: any) {
      console.error("❌ First-time tier selection failed:", err);
      Swal.fire({
        icon: "error",
        title: "Request Failed",
        text:
          err.response?.data?.message ||
          "An error occurred while submitting your tier selection.",
        confirmButtonColor: "#032352",
      });
    }
  ...
  if (tierId === "custom") {
    ...
    try {
      ...
      storage = backendData.tierStorage || storage;
