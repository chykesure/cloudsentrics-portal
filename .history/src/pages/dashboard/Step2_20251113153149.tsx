...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const isUpgradeTierPage = location.pathname === "/dashboard/upgrade-tier";
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // -------------------------------
    // Override currentTierInfo storage with backend value
    // -------------------------------
    try {
      const res = await axios.get(`${BASE_URL}/upgrade/request/by-email/${user.email || user.companyEmail}`);
      const backendData = res.data.data || {};
      if (currentTierInfo && backendData.tierStorage) {
        currentTierInfo = { ...currentTierInfo, storage: backendData.tierStorage };
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-3 sm:px-6 py-6 sm:py-10">
      ...
      >
        ...
        <div className="mb-6 sm:mb-8">
          {/* Render the heading only if it's not upgrade-tier page */}
          {!isUpgradeTierPage && (
          ...
          <div className="flex flex-col md:flex-row md:flex-wrap gap-3 sm:gap-4 text-base sm:text-xl">
            {/* Only show these if NOT on upgrade-tier page */}
            {!isUpgradeTierPage && (
