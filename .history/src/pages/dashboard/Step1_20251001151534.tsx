...
const Step1 = ({ goBack, jumpToStep }: StepProps) => {
  ...
  // For AWS accounts
  const [awsAccountCount, setAwsAccountCount] = useState<number | null>(null);
  const [customAwsCount, setCustomAwsCount] = useState<string>("");
  const [awsAliases, setAwsAliases] = useState<string[]>([
    "cloudsentrics-aws-", // A
    "cloudsentrics-aws-", // B
    "cloudsentrics-aws-", // C
    "cloudsentrics-aws-", // D
      ...
      {selectedOption === "aws" && (
        <div className="w-full px-2 sm:px-6 mb-12">
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 mt-10">
            ...
            <button
              onClick={() => {
                if (!selectedStorageCount) {
                  alert("Please select number of AWS accounts.");
                  return;
                }
                if (
                  Object.values(awsAliases).some(
                    (alias) => alias.trim() === ""
                  )
                ) {
                  alert("Please fill all AWS Alias fields.");
                  return;
                }
                jumpToStep?.(6);
              }}
