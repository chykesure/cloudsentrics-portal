...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  // Handle backend storage for CUSTOM
  ...
  // -------------------------------
  // Get target tier info or prompt for custom
  // -------------------------------
  let selectedTierInfo = tiers.find((t) => t.id === tierId) || null;
  ...
  if (tierId === "custom") {
    ...
    // Create a synthetic tier info object for Custom
    selectedTierInfo = {
      id: "custom",
      title: "CUSTOM",
  ...
  // -------------------------------
  // Handle same tier or downgrade check
  // -------------------------------
  ...
  try {
    ...
    setLoading(false);
    console.log("⏹️ Finished unified upgrade process");
  }
};
