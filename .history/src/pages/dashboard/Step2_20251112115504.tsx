...
const Step2 = ({ goBack, jumpToStep, formData, setFormData }: StepProps) => {
  ...
  const handleSelectTier = async (tierId: string) => {
    ...
    // ✅ Ensure current storage for CUSTOM tier is populated before anything else
    if (currentTierInfo?.title?.toUpperCase() === "CUSTOM") {
      try {
        const backendTierRes = await axios.get(
          `${BASE_URL}/upgrade/request/by-email/${user.email || user.companyEmail}`
        );
        console.log("📦 Backend response:", backendTierRes.data);
        const backendData = backendTierRes.data?.data || {};

        // Assign explicitly and safely
        currentTierInfo = {
          ...currentTierInfo,
          storage: backendData.tierStorage || "50 GB",
          title: backendData.tierTitle || currentTierInfo.title,
        };

        localStorage.setItem(
          "backendTierData",
          JSON.stringify({ tierStorage: currentTierInfo.storage })
        );
        console.log("✅ Updated currentTierInfo:", currentTierInfo);
      } catch (err) {
        console.warn("❌ Could not fetch backend custom tier data:", err);
    ...
    const currentStorageGB = parseStorageToGB(currentTierInfo);
