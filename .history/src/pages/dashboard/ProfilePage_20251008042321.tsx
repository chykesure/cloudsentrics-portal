    ...
    const fetchOnboardingProfile = async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:5000/api/onboarding/${email}` // <- new endpoint
        );
        ...
        setForm({
          ...
          tier: data.tier || "",       // fallback if you still want tier
          storage: data.storage || "", // fallback if you still want storage
        });
      } catch (err) {
