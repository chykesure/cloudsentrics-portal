...
const CustomerManagement = () => {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        "http://localhost:5002/api/admin/customers",
  ...
  const handleDelete = async (customer: Customer) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem("adminToken");
        await axios.delete(
          `http://localhost:5002/api/admin/customers/${customer._id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  // Helper to format aliases consistently
  const formatAliases = (aliasesObj?: Record<string, string>) => {
    if (!aliasesObj || Object.keys(aliasesObj).length === 0) return "-";
    return Object.entries(aliasesObj)
  ...
  const exportCSV = () => {
    ...
    const rows = customers.map((c, idx) => [
      ...
      formatAliases(c.aliases?.aliases),
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200 shadow">
            <table className="min-w-full text-sm border-collapse">
              <thead className="bg-[#032352] text-white">
                <tr>
                  {["S/N", "Customer ID", "Company Name", "Contacts", "AWS Accounts", "Aliases", "Created At", "Actions"].map((h, idx) => (
                    <th key={idx} className="py-3 px-4 text-left whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {customers.length ? (
                  customers.map((c, idx) => (
                    <tr key={c._id} className={`${idx % 2 === 0 ? "bg-gray-50" : "bg-white"} hover:bg-gray-100`}>
                      ...
                      <td className="py-2 px-4">
                        <div>
                          ...
                          <span className="text-xs text-gray-500">{c.companyInfo?.primaryEmail} | {c.companyInfo?.primaryPhone}</span>
                        </div>
                        <div className="mt-1">
                          <strong>Secondary:</strong> {c.companyInfo?.secondaryName || "-"} <br />
                          <span className="text-xs text-gray-500">{c.companyInfo?.secondaryEmail || "-"} | {c.companyInfo?.secondaryPhone || "-"}</span>
                        </div>
                      </td>
                      <td className="py-2 px-4 text-center">{c.awsSetup?.numberOfAccounts || 0}</td>
                      <td className="py-2 px-4">{formatAliases(c.aliases?.aliases)}</td>
                      <td className="py-2 px-4">{new Date(c.createdAt).toLocaleDateString()}</td>
                      <td className="py-2 px-4 flex gap-2">
                        <button onClick={() => setSelectedCustomer(c)} className="p-1 rounded hover:bg-blue-100">
                          <Eye className="w-4 h-4 text-blue-600" />
                        </button>
                        <button onClick={() => handleDelete(c)} className="p-1 rounded hover:bg-red-100">
                  ...
                  <tr>
                    <td colSpan={8} className="text-center py-6 text-gray-500 italic">No customers found</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            ...
            >
              <div className="bg-[#032352] p-4 flex justify-between items-center">
                <h3 className="text-white font-semibold">Customer Details - {selectedCustomer.customerId}</h3>
              ...
              <div className="p-6 space-y-3 text-sm">
                ...
                <p><strong>Aliases:</strong> {formatAliases(selectedCustomer.aliases?.aliases)}</p>
