...
const StaffManagement = () => {
  ...
  const fetchStaff = async () => {
    ...
    try {
      ...
      const res = await axios.get("https://onboardingportal.cloudsentrics.org/api/admin/staff", {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editingStaff) {
        await axios.put(
          `https://onboardingportal.cloudsentrics.org/api/admin/staff/${editingStaff._id}`,
          formData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } else {
        await axios.post("https://onboardingportal.cloudsentrics.org/api/admin/staff", formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      setShowModal(false);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      await axios.delete(`https://onboardingportal.cloudsentrics.org/api/admin/staff/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchStaff();
    } catch (err) {
  ...
  const toggleActive = async (s: Staff) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      await axios.put(
        `https://onboardingportal.cloudsentrics.org/api/admin/staff/${s._id}/toggle`,
        { active: !s.active },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchStaff();
