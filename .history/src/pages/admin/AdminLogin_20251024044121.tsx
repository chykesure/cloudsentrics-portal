...
const AdminLogin: FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Login payload:", { email: emailClean, password }); // Debug

      const res = await axios.post("https://api.onboardingportal.cloudsentrics.org/api/admin/login", {
        email: email.trim().toLowerCase(),
        password: password.trim(), // Trim here too!
      });
      ...
      if (res.data.admin?.role) {
        const normalizedRole = res.data.admin.role
          .trim()
          .toLowerCase()
          .replace(/[_\s]+/g, "-");
        ...
        console.warn("⚠️ No role found in response:", res.data.admin);
      }
