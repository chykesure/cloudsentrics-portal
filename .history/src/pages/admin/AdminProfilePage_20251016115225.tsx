...
const AdminProfilePage = () => {
    ...
    const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const newAvatarUrl = data.data.avatar.startsWith("http")
                ? data.data.avatar
    ...
    if (loading)
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-gray-100 to-gray-200">
                ...
                <p className="text-gray-700 text-lg mt-4 font-medium animate-pulse">Loading profile...</p>
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 relative overflow-hidden">
            ...
            >
                ...
                {/* INFO */}
                <h2 className="text-2xl sm:text-3xl font-bold text-[#032352]">{profile.email}</h2>
                <p className="text-gray-600 mt-1 text-base sm:text-lg">Role: {profile.role}</p>

                {/* DIVIDER */}
                <div className="mt-6 h-[1px] bg-gray-200 w-3/4"></div>

                {/* TIMESTAMP or PLACEHOLDER */}
                <p className="mt-3 text-sm text-gray-500">Member since 2025 • Last login: Today</p>
            </motion.div>
