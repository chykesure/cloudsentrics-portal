...
const UpgradeRequest = () => {
    ...
    const fetchRequests = async () => {
        try {
            setLoading(true);
            const res = await axios.get("http://localhost:5002/api/upgrade/all");
    ...
    const handleAction = async (id: string, action: "approve" | "deny") => {
        ...
        try {
            setActionLoading(id);
            const url = `http://localhost:5002/api/upgrade/${id}/${action}`;
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gray-100">
            ...
            <div className="flex justify-between items-center bg-white shadow-sm px-6 py-4 border-b">
                <h2 className="text-2xl font-bold text-gray-800">Upgrade Requests</h2>
                <button
                    onClick={refreshList}
