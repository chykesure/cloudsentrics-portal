  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem("adminToken");
        await axios.delete(
          `http://localhost:5002/api/admin/customers/${id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
                ...
                {customers.length ? (
                  customers.map((c, idx) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-4">{c.companyInfo?.companyEmail}</td>
                      <td className="py-2 px-4">{c.companyInfo?.primaryName}</td>
                      <td className="py-2 px-4">{c.companyInfo?.primaryEmail}</td>
                      <td className="py-2 px-4">{c.companyInfo?.primaryPhone}</td>
                      <td className="py-2 px-4">{c.companyInfo?.secondaryName || "-"}</td>
                      <td className="py-2 px-4">{c.companyInfo?.secondaryEmail || "-"}</td>
                      <td className="py-2 px-4">{c.companyInfo?.secondaryPhone || "-"}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            ...
            >
              ...
              <div className="p-6 space-y-3 text-sm">
                ...
                <p><strong>Secondary Contact:</strong> {selectedCustomer.companyInfo?.secondaryName || "N/A"}</p>
                <p><strong>Secondary Email:</strong> {selectedCustomer.companyInfo?.secondaryEmail || "N/A"}</p>
                <p><strong>Secondary Phone:</strong> {selectedCustomer.companyInfo?.secondaryPhone || "N/A"}</p>
