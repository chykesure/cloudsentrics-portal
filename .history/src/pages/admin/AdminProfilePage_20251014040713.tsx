...
const AdminProfilePage = () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        if (res.data.data.avatar) {
          setAvatar(res.data.data.avatar.startsWith("http") ? res.data.data.avatar : `http://localhost:5000${res.data.data.avatar}`);
        }
      } else {
  ...
  if (!profile) {
    return null; // extra safety, profile should never be null here
  }
  ...
  return (
    <div className="min-h-[80vh] w-full bg-gradient-to-b from-gray-100 to-gray-50 pb-16 relative">
      ...
      {/* Overlay Cards */}
      <div className="max-w-5xl mx-auto -mt-28 sm:-mt-36 px-4 sm:px-8 flex flex-col md:flex-row gap-8 justify-center items-start relative z-10">
        {/* Profile Card */}
        <motion.div
