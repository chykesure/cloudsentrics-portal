...
const AdminLogin: FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Login payload:", { email: emailClean, password: passwordClean });

      const res = await axios.post("https://api.onboardingportal.cloudsentrics.org/api/admin/login", {
        email: emailClean,
      ...
      // Normalize role
      if (admin?.role) {
        const normalizedRole = admin.role.trim().toLowerCase().replace(/[_\s]+/g, "-");
      ...
      if (admin?.passwordChangeRequired) {
        toast("You must change your password before logging in.", { icon: "⚠️" });
        setTimeout(() => navigate("/admin/change-password"), 1500);
        return;
      }
