...
const AdminDashboard = () => {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            // ✅ Normalize "Blocked" vs "In Review"
            const blockedOrReview = Number(data.blocked || data.inReview || 0);
            const hasBlockedData = !!data.blocked;

            setDashboardData({
                totalRequests: Number(data.totalRequests) || 0,
                todo: Number(data.todo) || 0,
                inProgress: Number(data.inProgress) || 0,
                blocked: hasBlockedData ? blockedOrReview : 0,
                inReview: !hasBlockedData ? blockedOrReview : 0,
                done: Number(data.done) || 0,
                issues: data.issues || [],
            });
            setHasBlocked(hasBlockedData);
        } catch (err) {
    ...
    // ✅ Dynamic label
    const reviewOrBlockedLabel = hasBlocked ? "Blocked" : "In Review";
    const reviewOrBlockedValue = hasBlocked
    ...
    const stats = [
        ...
        { label: reviewOrBlockedLabel, value: reviewOrBlockedValue, icon: AlertTriangle },
        { label: "Done", value: dashboardData.done, icon: CheckSquare },
    ];

    const countTotal = useCountUp(dashboardData.totalRequests);
    const countTodo = useCountUp(dashboardData.todo);
    const countInProgress = useCountUp(dashboardData.inProgress);
    const countBlocked = useCountUp(dashboardData.blocked);
    const countDone = useCountUp(dashboardData.done);

    const stats = [
        { label: "Total Requests", value: countTotal, icon: FileText },
        { label: "To Do", value: countTodo, icon: Clock },
        { label: "In Progress", value: countInProgress, icon: CheckCircle },
        { label: "Blocked", value: countBlocked, icon: AlertTriangle },
        { label: "Done", value: countDone, icon: CheckSquare },
    ];
    ...
    const chartData = [
        ...
        { name: reviewOrBlockedLabel, value: reviewOrBlockedValue, color: "#e11d48" },
                    ...
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 sm:mb-6 gap-3">
                        ...
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                            ...
                            >
                                ...
                                <option>{reviewOrBlockedLabel}</option>
                                <option>Done</option>
                            </select>
                            <button
                                onClick={fetchDashboardData}
