...
const StaffManagement = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editingStaff) {
        ...
        await axios.post("https://api.onboardingportal.cloudsentrics.org/api/admin/staff", formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      setShowModal(false);
  ...
  const toggleActive = async (s: Staff) => {
    // Only admin can toggle, and admin rows cannot be toggled
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200 shadow">
            <table className="min-w-full text-sm border-collapse">
              ...
              <tbody>
                {staff.length ? (
                  staff.map((s, idx) => {
                    const disabledToggle = currentUserRole !== "admin" || s.role === "admin";
