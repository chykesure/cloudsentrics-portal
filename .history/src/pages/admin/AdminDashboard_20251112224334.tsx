...
const AdminDashboard = () => {
  const [dashboardData, setDashboardData] = useState<DashboardData>({ issues: [] });
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "e /api/admin/dashboard",
        { headers: { Authorization: `Bearer ${token}` } }
      );

      const data = response.data || {};
      setDashboardData({ issues: data.issues || [] });
    } catch (err) {
  ...
  // ✅ Derive counts directly from Jira issues
  const issueCounts = useMemo(() => {
    const counts = {
      ...
      total: dashboardData.issues.length,
    };

    dashboardData.issues.forEach((issue) => {
      const status = issue.status?.toLowerCase();
      if (status.includes("to do")) counts.todo++;
      else if (status.includes("in progress")) counts.inProgress++;
      else if (status.includes("blocked")) counts.blocked++;
      else if (status.includes("done")) counts.done++;
    });
  ...
  const stats = [
    { label: "Total Requests", value: issueCounts.total, icon: FileText },
    { label: "To Do", value: issueCounts.todo, icon: Clock },
    { label: "In Progress", value: issueCounts.inProgress, icon: CheckCircle },
    { label: "Blocked", value: issueCounts.blocked, icon: AlertTriangle },
    { label: "Done", value: issueCounts.done, icon: CheckSquare },
  ];

  const chartData = [
    { name: "To Do", value: issueCounts.todo, color: "#f97316" },
    { name: "In Progress", value: issueCounts.inProgress, color: "#3b82f6" },
    { name: "Blocked", value: issueCounts.blocked, color: "#e11d48" },
    { name: "Done", value: issueCounts.done, color: "#16a34a" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 sm:mb-6 gap-3">
            <h3 className="text-base sm:text-lg font-semibold text-gray-800">Recent Jira Issues</h3>
              ...
              <thead className="bg-[#032352] text-white sticky top-0 shadow-sm">
                <tr>
                  {["Type", "Status", "Created", "Updated", "Action"].map((head, idx) => (
                    <th key={idx} className="py-3 px-3 font-medium text-left">
                      {head}
                    </th>
                  ))}
                </tr>
