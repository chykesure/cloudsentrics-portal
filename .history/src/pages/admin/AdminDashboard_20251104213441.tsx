...
const AdminDashboard = () => {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            // ✅ Normalize both “In Review” and “Blocked”
            const normalized = {
                totalRequests: Number(data.totalRequests) || 0,
                todo: Number(data.todo) || 0,
                inProgress: Number(data.inProgress) || 0,
                inReview: Number(data.inReview || data.blocked || 0), // handle mismatch
    ...
    // ✅ Detect if “Blocked” exists in issues
    const hasBlocked = dashboardData.issues?.some(
        (issue) => issue.status?.toLowerCase() === "blocked"
    );

    const dynamicLabel = hasBlocked ? "Blocked" : "In Review";
    ...
    const stats = [
        ...
        { label: dynamicLabel, value: dashboardData.inReview, icon: AlertTriangle },
    ...
    const chartData = [
        ...
        { name: dynamicLabel, value: dashboardData.inReview, color: "#facc15" },
