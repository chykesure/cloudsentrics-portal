...
const StaffManagement = () => {
  ...
  const isActive = s.role === 'admin' || s.email === currentAdminEmail ? true : s.active;
  ...
  const fetchStaff = async () => {
    ...
    try {
      ...
      // Ensure active is boolean for all users
      const normalizedStaff = (res.data.staff || []).map((s: Staff) => ({
        ...s,
        active: !!s.active,
      }));
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const payload = editingStaff
        ? formData
        : { ...formData, active: true }; // new staff active by default
      ...
      if (editingStaff) {
        ...
        await axios.post("https://api.onboardingportal.cloudsentrics.org/api/admin/staff", payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      setShowModal(false);
  ...
  const toggleActive = async (s: Staff) => {
    if (currentUserRole !== "admin" || s.role === "admin" || s.email === currentAdminEmail) return;
    ...
    try {
      ...
      setStaff(prev =>
        prev.map(st => (st._id === updatedStaff._id ? updatedStaff : st))
      );
    } catch (err) {
      console.error(err);
    }
                      ...
                      >
                        ...
                        <td className="py-2 px-4">
                          <button
                            ...
                            className={`p-1 rounded ${isActive ? "bg-green-100 hover:bg-green-200" : "bg-red-100 hover:bg-red-200"} ${disabledToggle ? "cursor-not-allowed" : "cursor-pointer"}`}
                          >
                            {isActive ? (
                        ...
                        <td className="py-2 px-4 flex gap-2">
                          <button
                            ...
                            disabled={currentUserRole !== "admin"}
                            className={`p-1 rounded hover:bg-gray-200 ${currentUserRole !== "admin" ? "opacity-50 cursor-not-allowed" : ""
                              }`}
                          >
                          ...
                          <button
                            ...
                            disabled={currentUserRole !== "admin"}
                            className={`p-1 rounded hover:bg-red-200 ${currentUserRole !== "admin" ? "opacity-50 cursor-not-allowed" : ""
                              }`}
                          >
