...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Normalize workflow status to work with both environments
      const normalizeStatusCounts = (data: any) => {
        const inReview = data.inReview || data.blocked || 0;
        return {
      ...
      setDashboardData(normalizeStatusCounts(data));
    } catch (err) {
  ...
  const stats = [
    ...
    {
      label: hasBlocked ? "Blocked" : "In Review",
  ...
  const chartData = [
    ...
    {
      name: hasBlocked ? "Blocked" : "In Review",
      value: dashboardData.inReview,
      color: "#facc15",
    },
          ...
          <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 sm:mb-6 gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              ...
              >
                ...
                <option>{hasBlocked ? "Blocked" : "In Review"}</option>
                <option>Done</option>
              </select>
              <button
                onClick={fetchDashboardData}
