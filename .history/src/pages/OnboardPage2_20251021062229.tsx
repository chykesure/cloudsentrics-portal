...
const OnboardingForm2: React.FC = () => {
  ...
  // ✅ Load Step1 and Step2 data (from location.state or sessionStorage)
  const storedStep1 = sessionStorage.getItem("formDataStep1");
  const storedAWS = sessionStorage.getItem("awsSetupData");

  const formDataStep1 =
    (location.state as any)?.formDataStep1 ||
    (storedStep1 ? JSON.parse(storedStep1) : null);

  const previousAwsData =
    (location.state as any)?.awsSetupData ||
    (storedAWS ? JSON.parse(storedAWS) : null);

  // ✅ Main form state
  const [selectedAccounts, setSelectedAccounts] = useState<number | null>(
    previousAwsData?.selectedAccounts ?? null
  );
  const [moreThanFive, setMoreThanFive] = useState<string>(
    previousAwsData?.moreThanFive ?? ""
  );
  const [aliases, setAliases] = useState<{ [key: string]: string }>(
    previousAwsData?.aliases ?? {}
  );
  const [bulkAliases, setBulkAliases] = useState<string>(
    previousAwsData?.bulkAliases ?? ""
  );

  // 🚨 Redirect if Step1 data is missing
  useEffect(() => {
    if (!formDataStep1) {
  ...
  const effectiveCount =
    selectedAccounts ?? (moreThanFive ? parseMoreThanFive(moreThanFive) : 0);
  const isHybridMode = effectiveCount > 6;
  const aliasKeys = Array.from(
    { length: Math.min(effectiveCount, 6) },
    (_, i) => String.fromCharCode(65 + i)
  );

  // 🧩 Maintain alias list length
  useEffect(() => {
  ...
  const allSixFilled =
    aliasKeys.length === 6 &&
    aliasKeys.every((k) => (aliases[k] ?? "").trim() !== "");
  const partialFilled =
    aliasKeys.length > 0 &&
    aliasKeys.every((k) => (aliases[k] ?? "").trim() !== "");
  const isNextDisabled =
    effectiveCount === 0 ||
    (isHybridMode
      ? !allSixFilled || bulkAliases.trim() === ""
      : !partialFilled);

  // 💾 Persist current data to sessionStorage every change
  useEffect(() => {
    const awsSetupData = {
      selectedAccounts,
      moreThanFive,
    ...
    sessionStorage.setItem("awsSetupData", JSON.stringify(awsSetupData));
    if (formDataStep1)
      sessionStorage.setItem("formDataStep1", JSON.stringify(formDataStep1));
  }, [selectedAccounts, moreThanFive, aliases, bulkAliases]);
  ...
  const handleBack = () => {
    ...
    sessionStorage.setItem("awsSetupData", JSON.stringify(awsSetupData));
    sessionStorage.setItem("formDataStep1", JSON.stringify(formDataStep1));
    navigate("/signup", { state: { formDataStep1 } });
  };
    ...
    <div className="relative flex flex-col md:flex-row min-h-screen w-full overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-10 w-full max-w-md mx-auto md:mx-0">
          ...
          <div className="flex items-center">
            <div className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full border-2 border-blue-700 text-white font-semibold">
              2
            </div>
          </div>
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div>
            <span className="block text-sm sm:text-lg font-semibold mb-1">
