...
const OnboardingForm3: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = { companyInfo: formDataStep1, awsSetup: awsSetupData, agreements };
    ...
    try {
      ...
      if (res.ok && data.success) {
        toast.success(`Onboarding saved! Customer ID: ${data.customerId}`, { position: "top-right" });
        setStep3Completed(true);
        setShowModal(true);
      } else {
        toast.error(data.message || "Unexpected error occurred", { position: "top-right" });
      }
    } catch (err) {
