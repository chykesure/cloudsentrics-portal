...
const OnboardingForm3: React.FC<OnboardingForm3Props> = ({ formDataStep1, userId }) => {
  ...
  // Try to get awsSetupData from location.state
  const awsSetupData = (location.state as { awsSetupData?: OnboardingData["awsSetup"] })?.awsSetupData;

  // Redirect to step2 if awsSetupData is missing
  useEffect(() => {
    if (!awsSetupData) {
      navigate("/signup/step2", { replace: true });
    }
  }, [awsSetupData, navigate]);
  ...
  // Example function to submit onboarding
const handleSubmit = async () => {
  const payload = {
    companyInfo: formDataStep1, // your collected company info from Step 1
    awsSetup: awsSetupData,     // your collected AWS setup data from Step 2
    agreements: agreementsState, // your agreements checkbox state
  };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      alert(data.error || "Failed to submit onboarding");
      return;
    }
    ...
    alert("Failed to submit onboarding. Check console for details.");
  }
};



  // Prevent rendering if awsSetupData is missing while redirecting
  if (!awsSetupData) return null;
