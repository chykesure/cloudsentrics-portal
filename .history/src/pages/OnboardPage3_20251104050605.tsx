...
const OnboardingForm3: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // allow awsSetupData to be optional when skipping step 2
  ...
  const handleSubmit = async () => {
    ...
    const safeAwsData = awsSetupData || {}; // fallback empty


    const payload = {
  companyInfo: formDataStep1,
  awsSetup: safeAwsData,
  agreements,
};


    try {
      const res = await fetch("https://api.onboardingportal.cloudsentrics.org/api/onboarding", {
        method: "POST",
      ...
      let data: any = {};
      try {
        data = await res.json(); // parse JSON always
      } catch {
      ...
      if (res.ok) {
        ...
        toast.error(data.error || "Validation error", { position: "top-right" });
      } else {
